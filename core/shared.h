...
// Window size, used to display window but also to cap the serialized position
...
// A code passed by the server when closing a client connection due to being
// full (max client count reached)
#define SERVER_FULL_CODE 42

// Client timeout in ticks (if no message received for this many ticks, client
...
typedef struct {
  ...
  unsigned int
      current_wave; // Numéro de la vague actuelle (0 = attente initiale)
...
void SerializeUpdateStateMessage(NetBuffer &buffer,
...
void SerializeConnectAcceptData(NetBuffer &buffer,
                                const ConnectAcceptData &data);
