...
#define BITS_REQUIRED(min, max)                                                \
  (min == max) ? 0 : GetRequiredNumberOfBitsFor(max - min)
...
#define ASSERT_VALUE_IN_RANGE(v, min, max)                                     \
  assert(v >= (int64_t)min && v <= (int64_t)max)
...
#define NBN_SerializeUInt(stream, v, min, max)                                 \
  ASSERTED_SERIALIZE(                                                          \
      (stream), v, min, max,                                                   \
      (stream)->serialize_uint_func((stream), (unsigned int *)&(v), min, max))
#define NBN_SerializeUInt64(stream, v)                                         \
  (stream)->serialize_uint64_func((stream), (uint64_t *)&(v))
#define NBN_SerializeInt(stream, v, min, max)                                  \
  ASSERTED_SERIALIZE((stream), v, min, max,                                    \
                     (stream)->serialize_int_func((stream), &(v), min, max))
#define NBN_SerializeFloat(stream, v, min, max, precision)                     \
  ASSERTED_SERIALIZE(                                                          \
      (stream), v, min, max,                                                   \
      (stream)->serialize_float_func((stream), &(v), min, max, precision))
#define NBN_SerializeBool(stream, v)                                           \
  ASSERTED_SERIALIZE((stream), v, 0, 1,                                        \
                     (stream)->serialize_bool_func((stream), &(v)))
#define NBN_SerializeString(stream, v, length)                                 \
  NBN_SerializeBytes((stream), v, length)
#define NBN_SerializeBytes(stream, v, length)                                  \
  (stream)->serialize_bytes_func((stream), (uint8_t *)v, length)
...
typedef int (*NBN_Stream_SerializeUInt)(NBN_Stream *, unsigned int *,
...
typedef int (*NBN_Stream_SerializeFloat)(NBN_Stream *, float *, float, float,
                                         int);
...
int NBN_ReadStream_SerializeUint(NBN_ReadStream *, unsigned int *, unsigned int,
                                 unsigned int);
int NBN_ReadStream_SerializeUint64(NBN_ReadStream *read_stream,
                                   uint64_t *value);
...
int NBN_WriteStream_SerializeUint(NBN_WriteStream *, unsigned int *,
                                  unsigned int, unsigned int);
int NBN_WriteStream_SerializeUint64(NBN_WriteStream *write_stream,
                                    uint64_t *value);
int NBN_WriteStream_SerializeInt(NBN_WriteStream *, int *, int, int);
int NBN_WriteStream_SerializeFloat(NBN_WriteStream *, float *, float, float,
                                   int);
...
int NBN_MeasureStream_SerializeUint(NBN_MeasureStream *, unsigned int *,
                                    unsigned int, unsigned int);
int NBN_MeasureStream_SerializeUint64(NBN_MeasureStream *measure_stream,
                                      unsigned int *value);
int NBN_MeasureStream_SerializeInt(NBN_MeasureStream *, int *, int, int);
int NBN_MeasureStream_SerializeFloat(NBN_MeasureStream *, float *, float, float,
                                     int);
int NBN_MeasureStream_SerializeBool(NBN_MeasureStream *, bool *);
int NBN_MeasureStream_SerializePadding(NBN_MeasureStream *);
int NBN_MeasureStream_SerializeBytes(NBN_MeasureStream *, uint8_t *,
                                     unsigned int);
...
#define NBN_MAX_MESSAGE_TYPES                                                  \
  255 /* Maximum value of uint8_t, see message header */
#define NBN_MESSAGE_RESEND_DELAY                                               \
  0.1 /* Number of seconds before a message is resent (reliable messages       \
         redundancy) */
...
/*
 * Holds the user message's data as well as a reference count for message
 * recycling
 */
...
#define NBN_RPC_MAX_PARAM_COUNT                                                \
  16 /* Maximum number of parameters a RPC signature can accept */
#define NBN_RPC_MAX 32 /* Maximum number of registered RPCs */
#define NBN_RPC_STRING_MAX_LENGTH                                              \
  256 /* Maximum length of a RPC string parameter */

/* Helper macros */
#define NBN_RPC_BuildSignature(pc, ...)                                        \
  ((NBN_RPC_Signature){.param_count = pc, .params = {__VA_ARGS__}})
#define NBN_RPC_Int(v)                                                         \
  ((NBN_RPC_Param){.type = NBN_RPC_PARAM_INT, .value = {.i = v}})
#define NBN_RPC_Float(v)                                                       \
  ((NBN_RPC_Param){.type = NBN_RPC_PARAM_FLOAT, .value = {.f = v}})
...
typedef void (*NBN_RPC_Func)(unsigned int,
                             NBN_RPC_Param[NBN_RPC_MAX_PARAM_COUNT],
                             NBN_ConnectionHandle sender);
 ...
 *
 * With the IPv4 header being 20 bytes and the UDP header being 8 bytes, the
 * payload of a UDP packet should be no larger than 1500 - 20 - 8 = 1472 bytes
 * to avoid fragmentation.
 */
#define NBN_PACKET_MAX_SIZE 1400
#define NBN_MAX_MESSAGES_PER_PACKET                                            \
  255 /* Maximum value of uint8_t, see packet header */

#define NBN_PACKET_HEADER_SIZE sizeof(NBN_PacketHeader)

/* Maximum size of packet's data (NBN_PACKET_MAX_DATA_SIZE +
 * NBN_PACKET_HEADER_SIZE = NBN_PACKET_MAX_SIZE) */
...
typedef struct NBN_Packet {
  ...
  struct NBN_Connection
      *sender; /* not serialized, fill by the network driver upon reception */
...
int NBN_Packet_InitRead(NBN_Packet *, NBN_Connection *,
                        uint8_t[NBN_PACKET_MAX_SIZE], unsigned int);
...
/* Chunk max size is the number of bytes of data a packet can hold minus the
 * size of a message header minus 2 bytes to hold the chunk id and total number
 * of chunks.
 */
#define NBN_MESSAGE_CHUNK_SIZE                                                 \
  (NBN_PACKET_MAX_DATA_SIZE - sizeof(NBN_MessageHeader) - 2)
#define NBN_MESSAGE_CHUNK_TYPE                                                 \
  (NBN_MAX_MESSAGE_TYPES - 1) /* Reserved message type for chunks */
...
#define NBN_CLIENT_CLOSED_MESSAGE_TYPE                                         \
  (NBN_MAX_MESSAGE_TYPES - 2) /* Reserved message type */
...
#define NBN_CLIENT_ACCEPTED_MESSAGE_TYPE                                       \
  (NBN_MAX_MESSAGE_TYPES - 3) /* Reserved message type */
...
int NBN_ClientAcceptedMessage_Serialize(NBN_ClientAcceptedMessage *,
                                        NBN_Stream *);
...
#define NBN_BYTE_ARRAY_MESSAGE_TYPE                                            \
  (NBN_MAX_MESSAGE_TYPES - 4) /* Reserved message type */
...
#define NBN_DISCONNECTION_MESSAGE_TYPE                                         \
  (NBN_MAX_MESSAGE_TYPES - 7) /* Reserved message type */
...
#define NBN_CONNECTION_REQUEST_MESSAGE_TYPE                                    \
  (NBN_MAX_MESSAGE_TYPES - 8) /* Reserved message type */
...
int NBN_ConnectionRequestMessage_Serialize(NBN_ConnectionRequestMessage *,
                                           NBN_Stream *);
...
#define NBN_RPC_MESSAGE_TYPE                                                   \
  (NBN_MAX_MESSAGE_TYPES - 9) /* Reserved message type */
...
/* IMPORTANT: if you add a library reserved channel below, make sure to update
 * NBN_LIBRARY_RESERVED_CHANNELS */
...
struct NBN_Channel {
  ...
  NBN_OutgoingMessage
      outgoing_message_pool[NBN_CHANNEL_OUTGOING_MESSAGE_POOL_SIZE];
...
int NBN_Channel_ReconstructMessageFromChunks(NBN_Channel *, NBN_Connection *,
                                             NBN_Message *);
...
void NBN_Channel_UpdateMessageLastSendTime(NBN_Channel *, NBN_Message *,
                                           double);

/*
   Unreliable ordered

   Guarantee that messages will be received in order, does not however guarantee
   that all message will be received when packets get lost. This is meant to be
   used for time critical messages when it does not matter that much if they end
   up getting lost. A good example would be game snaphosts when any newly
   received snapshot is more up to date than the previous one.
   */
...
/*
   ...
   Will guarantee that all messages will be received in order. Use this when you
   want to make sure a message will be received, for example for chat messages
   or initial game world state.
   */
...
struct NBN_Connection {
  ...
  double last_read_packets_time; /* Last time packets were read from the network
                                    driver */
  unsigned int downloaded_bytes; /* Keep track of bytes read from the socket
                                    (used for download bandwith calculation) */
  ...
  NBN_Channel *channels[NBN_MAX_CHANNELS]; /* Messages channeling (sending &
                                              receiving) */
...
int NBN_Connection_ProcessReceivedPacket(NBN_Connection *, NBN_Packet *,
                                         double);
int NBN_Connection_EnqueueOutgoingMessage(NBN_Connection *, NBN_Channel *,
                                          NBN_Message *);
...
int NBN_PacketSimulator_EnqueuePacket(NBN_PacketSimulator *, NBN_Packet *,
                                      NBN_Connection *);
...
#define NBN_GameClient_SetPing(v)                                              \
  NBN_LogInfo("NBN_Debug_SetPing: packet simulator is not enabled, ignore")
#define NBN_GameClient_SetJitter(v)                                            \
  NBN_LogInfo("NBN_Debug_SetJitter: packet simulator is not enabled, ignore")
#define NBN_GameClient_SetPacketLoss(v)                                        \
  NBN_LogInfo(                                                                 \
      "NBN_Debug_SetPacketLoss: packet simulator is not enabled, ignore")
#define NBN_GameClient_SetPacketDuplication(v)                                 \
  NBN_LogInfo("NBN_Debug_SetPacketDuplication: packet simulator is not "       \
              "enabled, ignore")
...
#define NBN_IsReservedMessage(type)                                            \
  (type == NBN_MESSAGE_CHUNK_TYPE || type == NBN_CLIENT_CLOSED_MESSAGE_TYPE || \
   type == NBN_CLIENT_ACCEPTED_MESSAGE_TYPE ||                                 \
   type == NBN_BYTE_ARRAY_MESSAGE_TYPE ||                                      \
   type == NBN_DISCONNECTION_MESSAGE_TYPE ||                                   \
   type == NBN_CONNECTION_REQUEST_MESSAGE_TYPE ||                              \
...
typedef struct NBN_GameClient {
  ...
  bool is_connected;
  uint8_t server_data[NBN_SERVER_DATA_MAX_SIZE]; /* Data sent by the server when
                                                    accepting the client's
                                                    connection */
  unsigned int
      server_data_len; /* Length of the received server data in bytes */
 ...
 *
 * @param protocol_name A unique protocol name, the clients and the server must
 * use the same one or they won't be able to communicate
 ...
 * @param host Host to connect to
 * @param port Port to connect to
 * @param data Array of bytes to send to the server upon connection (cannot
 * exceed NBN_CONNECTION_DATA_MAX_SIZE)
 * @param length length of the array of bytes
 *
 * @return 0 when successully started, -1 otherwise
 */
int NBN_GameClient_StartEx(const char *protocol_name, const char *host,
                           uint16_t port, uint8_t *data, unsigned int length);

/**
 * Disconnect from the server. The client can be restarted by calling
 * NBN_GameClient_Start or NBN_GameClient_StartWithData again.
 */
void NBN_GameClient_Stop(void);

/**
 * Read the server data that was received upon connection into a preallocated
 * buffer (the buffer must have a size of at least NBN_SERVER_DATA_MAX_SIZE
 * bytes).
 *
...
/**
 * Register a type of message on the game client, has to be called after
 * NBN_GameClient_Start.
 *
 *
 * @param msg_type A user defined message type, can be any value from 0 to 245
 * (245 to 255 are reserved by nbnet).
 * @param msg_builder The function responsible for building the message
 * @param msg_destructor The function responsible for destroying the message
 * (and releasing memory)
 * @param msg_serializer The function responsible for serializing the message
 */
void NBN_GameClient_RegisterMessage(uint8_t msg_type,
 ...
 *
 * @return The code of the polled event or NBN_NO_EVENT when there is no more
 * events.
 */
 ...
 *
 * It's recommended to use NBN_GameClient_SendUnreliableByteArray or
 * NBN_GameClient_SendReliableByteArray unless you really want to use a specific
 * channel.
 *
 ...
 * @return 0 when successful, -1 otherwise
 */
int NBN_GameClient_SendByteArray(uint8_t *bytes, unsigned int length,
                                 uint8_t channel_id);

/**
 * Send a message to the server on a given channel.
 *
 * It's recommended to use NBN_GameClient_SendUnreliableMessage or
 * NBN_GameClient_SendReliableMessage unless you really want to use a specific
 * channel.
 *
 * @param msg_type The type of message to send; it needs to be registered (see
 ...
 * @return 0 when successful, -1 otherwise
 */
int NBN_GameClient_SendMessage(uint8_t msg_type, uint8_t channel_id,
                               void *msg_data);
 ...
 * @param msg_type The type of message to send; it needs to be registered (see
 * NBN_GameClient_RegisterMessage)
 * @param msg_data A pointer to the message to send (pointing to user code
 * memory)
 *
 * @return 0 when successful, -1 otherwise
 */
 ...
 */
NBN_Connection *NBN_GameClient_CreateServerConnection(int driver_id,
                                                      void *driver_data,
                                                      uint32_t protocol_id);
 ...
 *
 * @return The code used by the server when closing the connection or -1 (the
 * default code)
 */
 ...
 */
int NBN_GameClient_RegisterRPC(unsigned int id, NBN_RPC_Signature signature,
                               NBN_RPC_Func func);

/**
 * Call a previously registered RPC on the game server.
 *
 * @param id The ID of the RPC to execute on the game server (must be a
 * registered ID)
 */
...
/**
 * Register a type of message on the game server, has to be called after
 ...
 * @param msg_serializer The function responsible for serializing the message
 */
void NBN_GameServer_RegisterMessage(uint8_t msg_type,
                                    NBN_MessageBuilder msg_builder,
                                    NBN_MessageDestructor msg_destructor,
                                    NBN_MessageSerializer msg_serializer);
 ...
 * @return 0 when successful, -1 otherwise
 */
int NBN_GameServer_CloseClientWithCode(NBN_ConnectionHandle connection_handle,
                                       int code);
 ...
 * @return 0 when successful, -1 otherwise
 */
int NBN_GameServer_SendByteArrayTo(NBN_ConnectionHandle connection_handle,
                                   uint8_t *bytes, unsigned int length,
                                   uint8_t channel_id);

/**
 * Send a message to a client on a given channel.
 *
 * It's recommended to use NBN_GameServer_SendUnreliableMessageTo or
 * NBN_GameServer_SendReliableMessageTo unless you really want to use a specific
 * channel.
 *
 * @param connection_handle The connection to send the message to
 * @param msg_type The type of message to send; it needs to be registered (see
...
int NBN_GameServer_SendMessageTo(NBN_ConnectionHandle connection_handle,
                                 uint8_t msg_type, uint8_t channel_id,
                                 void *msg_data);
 ...
 * @return 0 when successful, -1 otherwise
 */
int NBN_GameServer_SendUnreliableMessageTo(
    NBN_ConnectionHandle connection_handle, uint8_t msg_type, void *msg_data);
 ...
 * @return 0 when successful, -1 otherwise
 */
int NBN_GameServer_SendReliableMessageTo(NBN_ConnectionHandle connection_handle,
                                         uint8_t msg_type, void *msg_data);
 ...
 * @return 0 when successful, -1 otherwise
 */
int NBN_GameServer_SendUnreliableByteArrayTo(
    NBN_ConnectionHandle connection_handle, uint8_t *bytes,
    unsigned int length);
 ...
 * @return 0 when successful, -1 otherwise
 */
int NBN_GameServer_SendReliableByteArrayTo(
    NBN_ConnectionHandle connection_handle, uint8_t *bytes,
    unsigned int length);

/**
 * Accept the last client connection request and send a blob of data to the
 * client. The client can read that data using the NBN_GameClient_ReadServerData
 * function. If you do not wish to send any data to the client upon accepting
 * his connection, use the NBN_GameServer_AcceptIncomingConnection function
 * instead.
 *
 ...
 * @return 0 when successful, -1 otherwise
 */
int NBN_GameServer_AcceptIncomingConnectionWithData(uint8_t *data,
                                                    unsigned int length);
...
/**
 * Reject the last client connection request without any specific code (default
 * code is -1)
 *
 * Call this function after receiving a NBN_NEW_CONNECTION event.
 *
...
/**
 * Read the last connection data into a preallocated buffer. The target buffer
 * must have a length of at least NBN_CONNECTION_DATA_MAX_SIZE bytes.
 *
 ...
 * Retrieve the info about the last received message.
 *
 * Call this function when receiving a NBN_CLIENT_MESSAGE_RECEIVED event to
 ...
 * @return true if packet encryption is enabled, false otherwise
 */
int NBN_GameServer_RegisterRPC(unsigned int id, NBN_RPC_Signature signature,
                               NBN_RPC_Func func);
 ...
 */
int NBN_GameServer_CallRPC(unsigned int id,
                           NBN_ConnectionHandle connection_handle, ...);
...
typedef int (*NBN_Driver_ServerSendPacketToFunc)(NBN_Packet *,
                                                 NBN_Connection *);
...
/**
 * Register a new network driver, at least one network driver has to be
 * registered.
 *
 * @param id ID of the driver, must be unique and within 0 and NBN_MAX_DRIVERS
 * @param name Name of the driver
 * @param signature Driver implementation (structure containing all driver
 * implementation function pointers)
 */
void NBN_Driver_Register(int id, const char *name,
                         NBN_DriverImplementation implementation);

/**
 * Let nbnet know about specific network events happening within a network
 * driver.
 *
 * @param ev Event type
...
#define SEQUENCE_NUMBER_GT(seq1, seq2)                                         \
  ((seq1 > seq2 && (seq1 - seq2) <= 32767) ||                                  \
   (seq1 < seq2 && (seq2 - seq1) >= 32767))
...
static void NBN_ConnectionVector_Grow(NBN_ConnectionVector *vector,
                                      unsigned int new_capacity);

static NBN_ConnectionVector *NBN_ConnectionVector_Create(void) {
  NBN_ConnectionVector *vector =
      (NBN_ConnectionVector *)NBN_Allocator(sizeof(NBN_ConnectionVector));
...
static void NBN_ConnectionVector_Add(NBN_ConnectionVector *vector,
                                     NBN_Connection *conn) {
  ...
  if (vector->connections[position]) {
    NBN_LogError(
        "Failed to add connection (id: %d) to vector: position %d is not empty",
        conn->id, position);
    NBN_Abort();
  }
...
static uint32_t NBN_ConnectionVector_RemoveAt(NBN_ConnectionVector *vector,
  ...
  last_conn->vector_pos = position; // last connection in the vector is moved to
...
static void NBN_ConnectionVector_Grow(NBN_ConnectionVector *vector,
                                      unsigned int new_capacity) {
  vector->connections = (NBN_Connection **)NBN_Reallocator(
      vector->connections, sizeof(NBN_Connection *) * new_capacity);
...
static void NBN_ConnectionTable_InsertEntry(NBN_ConnectionTable *table,
                                            unsigned int slot,
                                            NBN_Connection *conn);
static void NBN_ConnectionTable_RemoveEntry(NBN_ConnectionTable *table,
                                            unsigned int slot);
static unsigned int NBN_ConnectionTable_Hash(int hash);
static void NBN_ConnectionTable_Grow(NBN_ConnectionTable *table,
                                     unsigned int new_capacity);

static NBN_ConnectionTable *NBN_ConnectionTable_Create(void) {
  NBN_ConnectionTable *table =
      (NBN_ConnectionTable *)NBN_Allocator(sizeof(NBN_ConnectionTable));
...
static void NBN_ConnectionTable_Destroy(NBN_ConnectionTable *table) {
  // Make sure we don't destroy the actual connections as they will be destroyed
  // with the connection vector
  NBN_Deallocator(table->connections);
  NBN_Deallocator(table);
}

static void NBN_ConnectionTable_Add(NBN_ConnectionTable *table,
                                    NBN_Connection *conn) {
...
static bool NBN_ConnectionTable_Remove(NBN_ConnectionTable *table,
                                       uint32_t id) {
...
static NBN_Connection *NBN_ConnectionTable_Get(NBN_ConnectionTable *table,
                                               uint32_t id) {
  unsigned int hash = NBN_ConnectionTable_Hash(id);
  unsigned int slot = hash % table->capacity;
  NBN_Connection *conn = table->connections[slot];
...
static void NBN_ConnectionTable_Grow(NBN_ConnectionTable *table,
  ...
  NBN_Connection **new_connections_array =
      (NBN_Connection **)NBN_Allocator(sizeof(NBN_Connection *) * new_capacity);
  ...
  if (old_connections_array)
    NBN_Deallocator(old_connections_array);
}
...
static void MemoryManager_Init(void) {
  ...
  nbn_mem_manager.mem_sizes[NBN_MEM_BYTE_ARRAY_MESSAGE] =
      sizeof(NBN_ByteArrayMessage);
  nbn_mem_manager.mem_sizes[NBN_MEM_CONNECTION] = sizeof(NBN_Connection);

#if defined(NBN_DEBUG) && defined(NBN_USE_PACKET_SIMULATOR)
  nbn_mem_manager.mem_sizes[NBN_MEM_PACKET_SIMULATOR_ENTRY] =
      sizeof(NBN_PacketSimulatorEntry);
#endif
#else
  NBN_LogDebug("MemoryManager_Init with pooling!");

  MemPool_Init(&nbn_mem_manager.mem_pools[NBN_MEM_MESSAGE_CHUNK],
               sizeof(NBN_MessageChunk), 256);
  MemPool_Init(&nbn_mem_manager.mem_pools[NBN_MEM_BYTE_ARRAY_MESSAGE],
               sizeof(NBN_ByteArrayMessage), 256);
  MemPool_Init(&nbn_mem_manager.mem_pools[NBN_MEM_CONNECTION],
               sizeof(NBN_Connection), 16);

#if defined(NBN_DEBUG) && defined(NBN_USE_PACKET_SIMULATOR)
  MemPool_Init(&nbn_mem_manager.mem_pools[NBN_MEM_PACKET_SIMULATOR_ENTRY],
               sizeof(NBN_PacketSimulatorEntry), 32);
#endif
#endif /* NBN_DISABLE_MEMORY_POOLING */
}
...
static void MemPool_Init(NBN_MemPool *pool, size_t block_size,
...
static void MemPool_Grow(NBN_MemPool *pool, unsigned int block_count) {
  pool->blocks = (uint8_t **)NBN_Reallocator(pool->blocks,
                                             sizeof(uint8_t *) * block_count);

  for (unsigned int i = 0; i < block_count - pool->block_count; i++)
    pool->blocks[pool->block_idx + i] =
        (uint8_t *)NBN_Allocator(pool->block_size);
...
void NBN_BitReader_Init(NBN_BitReader *bit_reader, uint8_t *buffer,
                        unsigned int size) {
...
int NBN_BitReader_Read(NBN_BitReader *bit_reader, Word *word,
                       unsigned int number_of_bits) {
  *word = 0;

  if (number_of_bits > bit_reader->scratch_bits_count) {
    unsigned int needed_bytes =
        (number_of_bits - bit_reader->scratch_bits_count - 1) / 8 + 1;
...
static void BitReader_ReadFromBuffer(NBN_BitReader *bit_reader) {
  unsigned int bytes_count =
      MIN(bit_reader->size - bit_reader->byte_cursor, WORD_BYTES);
  Word word = 0;
...
void NBN_BitWriter_Init(NBN_BitWriter *bit_writer, uint8_t *buffer,
                        unsigned int size) {
...
int NBN_BitWriter_Write(NBN_BitWriter *bit_writer, Word value,
                        unsigned int number_of_bits) {
...
static int BitWriter_FlushScratchBits(NBN_BitWriter *bit_writer,
                                      unsigned int number_of_bits) {
...
void NBN_ReadStream_Init(NBN_ReadStream *read_stream, uint8_t *buffer,
                         unsigned int size) {
  read_stream->base.type = NBN_STREAM_READ;
  read_stream->base.serialize_uint_func =
      (NBN_Stream_SerializeUInt)NBN_ReadStream_SerializeUint;
  read_stream->base.serialize_uint64_func =
      (NBN_Stream_SerializeUInt64)NBN_ReadStream_SerializeUint64;
  read_stream->base.serialize_int_func =
      (NBN_Stream_SerializeInt)NBN_ReadStream_SerializeInt;
  read_stream->base.serialize_float_func =
      (NBN_Stream_SerializeFloat)NBN_ReadStream_SerializeFloat;
  read_stream->base.serialize_bool_func =
      (NBN_Stream_SerializeBool)NBN_ReadStream_SerializeBool;
  read_stream->base.serialize_padding_func =
      (NBN_Stream_SerializePadding)NBN_ReadStream_SerializePadding;
  read_stream->base.serialize_bytes_func =
      (NBN_Stream_SerializeBytes)NBN_ReadStream_SerializeBytes;

  NBN_BitReader_Init(&read_stream->bit_reader, buffer, size);
}

int NBN_ReadStream_SerializeUint(NBN_ReadStream *read_stream,
                                 unsigned int *value, unsigned int min,
                                 unsigned int max) {
  assert(min <= max);

  if (NBN_BitReader_Read(&read_stream->bit_reader, value,
                         BITS_REQUIRED(min, max)) < 0)
    return NBN_ERROR;
...
int NBN_ReadStream_SerializeInt(NBN_ReadStream *read_stream, int *value,
                                int min, int max) {
  assert(min <= max);
  ...
  *value = abs(*value);
  ...
  if (NBN_ReadStream_SerializeUint(read_stream, (unsigned int *)value,
                                   (min < 0 && max > 0) ? 0 : abs_min,
                                   abs_max) < 0)
    return NBN_ERROR;
...
int NBN_ReadStream_SerializeFloat(NBN_ReadStream *read_stream, float *value,
                                  float min, float max, int precision) {
  assert(min <= max);
...
int NBN_ReadStream_SerializeBytes(NBN_ReadStream *read_stream, uint8_t *bytes,
                                  unsigned int length) {
  if (length == 0)
    return NBN_ERROR;
  ...
  if (length * 8 <= bit_reader->scratch_bits_count && length <= sizeof(Word)) {
    ...
    // reading is done word by word and in this case, the start of byte array
    // has already been read therefore we need to "roll back" the byte cursor so
    // it is at the very begining of the byte array inside the buffer
...
void NBN_WriteStream_Init(NBN_WriteStream *write_stream, uint8_t *buffer,
                          unsigned int size) {
  write_stream->base.type = NBN_STREAM_WRITE;
  write_stream->base.serialize_uint_func =
      (NBN_Stream_SerializeUInt)NBN_WriteStream_SerializeUint;
  write_stream->base.serialize_uint64_func =
      (NBN_Stream_SerializeUInt64)NBN_WriteStream_SerializeUint64;
  write_stream->base.serialize_int_func =
      (NBN_Stream_SerializeInt)NBN_WriteStream_SerializeInt;
  write_stream->base.serialize_float_func =
      (NBN_Stream_SerializeFloat)NBN_WriteStream_SerializeFloat;
  write_stream->base.serialize_bool_func =
      (NBN_Stream_SerializeBool)NBN_WriteStream_SerializeBool;
  write_stream->base.serialize_padding_func =
      (NBN_Stream_SerializePadding)NBN_WriteStream_SerializePadding;
  write_stream->base.serialize_bytes_func =
      (NBN_Stream_SerializeBytes)NBN_WriteStream_SerializeBytes;

  NBN_BitWriter_Init(&write_stream->bit_writer, buffer, size);
}

int NBN_WriteStream_SerializeUint(NBN_WriteStream *write_stream,
                                  unsigned int *value, unsigned int min,
                                  unsigned int max) {
  assert(min <= max);
  assert(*value >= min && *value <= max);

  if (NBN_BitWriter_Write(&write_stream->bit_writer, *value - min,
                          BITS_REQUIRED(min, max)) < 0)
    return NBN_ERROR;
...
int NBN_WriteStream_SerializeInt(NBN_WriteStream *write_stream, int *value,
                                 int min, int max) {
  assert(min <= max);
  ...
  *value = abs(*value);
  ...
  if (NBN_WriteStream_SerializeUint(write_stream, (unsigned int *)value,
                                    (min < 0 && max > 0) ? 0 : abs_min,
                                    abs_max) < 0)
    return NBN_ERROR;
...
int NBN_WriteStream_SerializeFloat(NBN_WriteStream *write_stream, float *value,
                                   float min, float max, int precision) {
  assert(min <= max);
...
int NBN_WriteStream_SerializeBytes(NBN_WriteStream *write_stream,
                                   uint8_t *bytes, unsigned int length) {
  if (length == 0)
    return NBN_ERROR;
  ...
  // make sure everything has been flushed to the buffer before writing the byte
  // array
...
void NBN_MeasureStream_Init(NBN_MeasureStream *measure_stream) {
  ...
  measure_stream->base.serialize_uint_func =
      (NBN_Stream_SerializeUInt)NBN_MeasureStream_SerializeUint;
  measure_stream->base.serialize_uint64_func =
      (NBN_Stream_SerializeUInt64)NBN_MeasureStream_SerializeUint64;
  measure_stream->base.serialize_int_func =
      (NBN_Stream_SerializeInt)NBN_MeasureStream_SerializeInt;
  measure_stream->base.serialize_float_func =
      (NBN_Stream_SerializeFloat)NBN_MeasureStream_SerializeFloat;
  measure_stream->base.serialize_bool_func =
      (NBN_Stream_SerializeBool)NBN_MeasureStream_SerializeBool;
  measure_stream->base.serialize_padding_func =
      (NBN_Stream_SerializePadding)NBN_MeasureStream_SerializePadding;
  measure_stream->base.serialize_bytes_func =
      (NBN_Stream_SerializeBytes)NBN_MeasureStream_SerializeBytes;

  measure_stream->number_of_bits = 0;
}

int NBN_MeasureStream_SerializeUint(NBN_MeasureStream *measure_stream,
                                    unsigned int *value, unsigned int min,
                                    unsigned int max) {
  (void)*value;
...
int NBN_MeasureStream_SerializeInt(NBN_MeasureStream *measure_stream,
...
int NBN_MeasureStream_SerializeFloat(NBN_MeasureStream *measure_stream,
...
int NBN_MeasureStream_SerializeBool(NBN_MeasureStream *measure_stream,
                                    bool *value) {
  (void)value;
...
int NBN_MeasureStream_SerializeBytes(NBN_MeasureStream *measure_stream,
                                     uint8_t *bytes, unsigned int length) {
  (void)bytes;
...
void NBN_Packet_InitWrite(NBN_Packet *packet, uint32_t protocol_id,
                          uint16_t seq_number, uint16_t ack,
                          uint32_t ack_bits) {
  ...
  NBN_WriteStream_Init(&packet->w_stream,
                       packet->buffer + NBN_PACKET_HEADER_SIZE,
                       NBN_PACKET_MAX_DATA_SIZE);
  NBN_MeasureStream_Init(&packet->m_stream);
}

int NBN_Packet_InitRead(NBN_Packet *packet, NBN_Connection *sender,
                        uint8_t buffer[NBN_PACKET_MAX_SIZE],
                        unsigned int size) {
  ...
  if (Packet_SerializeHeader(&packet->header, (NBN_Stream *)&header_r_stream) <
      0)
    return NBN_ERROR;

  NBN_ReadStream_Init(&packet->r_stream,
                      packet->buffer + NBN_PACKET_HEADER_SIZE, packet->size);

  return 0;
}

uint32_t NBN_Packet_ReadProtocolId(uint8_t buffer[NBN_PACKET_MAX_SIZE],
                                   unsigned int size) {
...
int NBN_Packet_WriteMessage(NBN_Packet *packet, NBN_Message *message,
  ...
  if (NBN_Message_SerializeHeader(&message->header,
                                  (NBN_Stream *)&packet->w_stream) < 0)
    return NBN_PACKET_WRITE_ERROR;

  if (NBN_Message_SerializeData(message, (NBN_Stream *)&packet->w_stream,
                                msg_serializer) < 0)
    return NBN_PACKET_WRITE_ERROR;
...
int NBN_Packet_Seal(NBN_Packet *packet, NBN_Connection *connection) {
  ...
  NBN_WriteStream_Init(&header_w_stream, packet->buffer,
                       NBN_PACKET_HEADER_SIZE);

  if (Packet_SerializeHeader(&packet->header, (NBN_Stream *)&header_w_stream) <
      0)
    return NBN_ERROR;
...
static int Packet_SerializeHeader(NBN_PacketHeader *header,
                                  NBN_Stream *stream) {
  ...
  NBN_SerializeBytes(stream, &header->messages_count,
                     sizeof(header->messages_count));
...
int NBN_Message_SerializeHeader(NBN_MessageHeader *message_header,
  ...
  NBN_SerializeBytes(stream, &message_header->type,
                     sizeof(message_header->type));
  NBN_SerializeBytes(stream, &message_header->channel_id,
                     sizeof(message_header->channel_id));
...
int NBN_Message_Measure(NBN_Message *message, NBN_MeasureStream *m_stream,
  ...
  if (NBN_Message_SerializeData(message, (NBN_Stream *)m_stream,
                                msg_serializer) < 0)
    return NBN_ERROR;
...
NBN_MessageChunk *NBN_MessageChunk_Create(void) {
  NBN_MessageChunk *chunk =
      (NBN_MessageChunk *)MemoryManager_Alloc(NBN_MEM_MESSAGE_CHUNK);
...
NBN_ClientClosedMessage *NBN_ClientClosedMessage_Create(void) {
  return (NBN_ClientClosedMessage *)NBN_Allocator(
      sizeof(NBN_ClientClosedMessage));
}
...
int NBN_ClientClosedMessage_Serialize(NBN_ClientClosedMessage *msg,
                                      NBN_Stream *stream) {
...
NBN_ClientAcceptedMessage *NBN_ClientAcceptedMessage_Create(void) {
  return (NBN_ClientAcceptedMessage *)NBN_Allocator(
      sizeof(NBN_ClientAcceptedMessage));
}
...
int NBN_ClientAcceptedMessage_Serialize(NBN_ClientAcceptedMessage *msg,
                                        NBN_Stream *stream) {
...
NBN_ByteArrayMessage *NBN_ByteArrayMessage_Create(void) {
  return (NBN_ByteArrayMessage *)MemoryManager_Alloc(
      NBN_MEM_BYTE_ARRAY_MESSAGE);
}
...
int NBN_ByteArrayMessage_Serialize(NBN_ByteArrayMessage *msg,
                                   NBN_Stream *stream) {
...
void *NBN_DisconnectionMessage_Create(void) { return NULL; }

void NBN_DisconnectionMessage_Destroy(void *msg) { NBN_Deallocator(msg); }
...
NBN_ConnectionRequestMessage *NBN_ConnectionRequestMessage_Create(void) {
  return (NBN_ConnectionRequestMessage *)NBN_Allocator(
      sizeof(NBN_ConnectionRequestMessage));
}
...
int NBN_ConnectionRequestMessage_Serialize(NBN_ConnectionRequestMessage *msg,
                                           NBN_Stream *stream) {
...
void NBN_RPC_Message_Destroy(NBN_RPC_Message *msg) { NBN_Deallocator(msg); }
...
int NBN_RPC_Message_Serialize(NBN_RPC_Message *msg, NBN_Stream *stream) {
  ...
  for (unsigned int i = 0; i < msg->param_count; i++) {
    ...
    if (p->type == NBN_RPC_PARAM_INT) {
      ...
      if (stream->type == NBN_STREAM_WRITE ||
          stream->type == NBN_STREAM_MEASURE) {
        int l = strlen(p->value.s);

        assert(l + 1 <= NBN_RPC_STRING_MAX_LENGTH); // make sure we have a spot
...
static NBN_OutgoingMessage *
Endpoint_CreateOutgoingMessage(NBN_Endpoint *, NBN_Channel *, uint8_t, void *);
...
static void Connection_InitOutgoingPacket(NBN_Connection *, NBN_Packet *,
                                          NBN_PacketEntry **);
static NBN_PacketEntry *Connection_InsertOutgoingPacketEntry(NBN_Connection *,
                                                             uint16_t);
static bool Connection_InsertReceivedPacketEntry(NBN_Connection *, uint16_t);
static NBN_PacketEntry *Connection_FindSendPacketEntry(NBN_Connection *,
                                                       uint16_t);
static bool Connection_IsPacketReceived(NBN_Connection *, uint16_t);
static int Connection_SendPacket(NBN_Connection *, NBN_Packet *,
                                 NBN_PacketEntry *, double);
static int Connection_ReadNextMessageFromStream(NBN_Connection *,
                                                NBN_ReadStream *,
                                                NBN_Message *);
static int Connection_ReadNextMessageFromPacket(NBN_Connection *, NBN_Packet *,
                                                NBN_Message *);
...
static NBN_RPC_Message *Connection_BuildRPC(NBN_Connection *, NBN_RPC *,
                                            va_list);
static void Connection_HandleReceivedRPC(NBN_ConnectionHandle, NBN_Endpoint *,
                                         NBN_RPC_Message *);

NBN_Connection *NBN_Connection_Create(uint32_t id, uint32_t protocol_id,
                                      NBN_Endpoint *endpoint,
                                      NBN_Driver *driver, void *driver_data) {
  NBN_Connection *connection =
      (NBN_Connection *)MemoryManager_Alloc(NBN_MEM_CONNECTION);
...
int NBN_Connection_ProcessReceivedPacket(NBN_Connection *connection,
                                         NBN_Packet *packet, double time) {
  if (Connection_DecodePacketHeader(connection, packet, time) < 0) {
    NBN_LogError("Failed to decode packet %d header",
  ...
  if (!Connection_InsertReceivedPacketEntry(connection,
                                            packet->header.seq_number))
    return 0;

  if (SEQUENCE_NUMBER_GT(packet->header.seq_number,
  ...
  for (int i = 0; i < packet->header.messages_count; i++) {
    ...
    if (Connection_ReadNextMessageFromPacket(connection, packet, &message) <
        0) {
    ...
    if (channel->AddReceivedMessage(channel, &message)) {
      NBN_LogTrace("Received message %d on channel %d : added to recv queue",
                   message.header.id, channel->id);

#ifdef NBN_DEBUG
      if (connection->debug_callbacks.OnMessageAddedToRecvQueue)
        connection->debug_callbacks.OnMessageAddedToRecvQueue(connection,
                                                              &message);
#endif
    } else {
...
int NBN_Connection_EnqueueOutgoingMessage(NBN_Connection *connection,
  ...
  NBN_LogTrace("Enqueue message of type %d on channel %d", message->header.type,
               channel->id);
...
int NBN_Connection_FlushSendQueue(NBN_Connection *connection, double time) {
  ...
  for (unsigned int i = 0; i < NBN_MAX_CHANNELS; i++) {
    ...
    NBN_LogTrace("Flushing channel %d (message count: %d)", channel->id,
                 channel->outgoing_message_count);

    NBN_Message *message;
    unsigned int j = 0;

    while (j < channel->outgoing_message_count &&
           sent_packet_count < NBN_CONNECTION_MAX_SENT_PACKET_COUNT &&
           (message = channel->GetNextOutgoingMessage(channel, time)) != NULL) {
      bool message_sent = false;
      NBN_MessageSerializer msg_serializer =
          connection->endpoint->message_serializers[message->header.type];
      ...
      if (ret == NBN_PACKET_WRITE_OK) {
        ...
        if (Connection_SendPacket(connection, &packet, packet_entry, time) <
            0) {
        ...
        NBN_LogError("Failed to write message %d of type %d to packet %d",
                     message->header.id, message->header.type,
                     packet.header.seq_number);
      ...
      if (message_sent) {
        NBN_LogTrace("Message %d added to packet %d", message->header.id,
                     packet.header.seq_number);
  ...
  if (Connection_SendPacket(connection, &packet, packet_entry, time) < 0) {
    NBN_LogError("Failed to send packet %d to connection %d",
                 packet.header.seq_number, connection->id);
...
int NBN_Connection_InitChannel(NBN_Connection *connection,
                               NBN_Channel *channel) {
  channel->connection = connection;
  channel->read_chunk_buffer =
      (uint8_t *)NBN_Allocator(NBN_CHANNEL_RW_CHUNK_BUFFER_INITIAL_SIZE);
  channel->write_chunk_buffer =
      (uint8_t *)NBN_Allocator(NBN_CHANNEL_RW_CHUNK_BUFFER_INITIAL_SIZE);
  ...
  NBN_LogDebug("Initialized channel %d for connection %d", channel->id,
               connection->id);
  return 0;
}
...
bool NBN_Connection_CheckIfStale(NBN_Connection *connection, double time) {
  ...
  (void)connection;
  (void)time;
  /* When testing under bad network conditions (in soak test for instance), we
     don't want to deal with stale connections */
  return false;
#else
  return time - connection->last_recv_packet_time >
         NBN_CONNECTION_STALE_TIME_THRESHOLD;
#endif
}

static int Connection_DecodePacketHeader(NBN_Connection *connection,
                                         NBN_Packet *packet, double time) {
  ...
  for (unsigned int i = 0; i < 32; i++) {
    ...
    if (Connection_AckPacket(connection, packet->header.ack - (i + 1), time) <
        0) {
      NBN_LogError("Failed to ack packet %d", packet->header.seq_number);
...
static uint32_t Connection_BuildPacketAckBits(NBN_Connection *connection) {
  ...
  for (int i = 0; i < 32; i++) {
    /*
       when last_received_packet_seq_number is lower than 32, the value of
       acked_packet_seq_number will eventually wrap around, which means the
       packets from before the wrap around will naturally be acked
       */

    uint16_t acked_packet_seq_number =
        connection->last_received_packet_seq_number - (i + 1);
...
static int Connection_AckPacket(NBN_Connection *connection,
                                uint16_t ack_packet_seq_number, double time) {
  NBN_PacketEntry *packet_entry =
      Connection_FindSendPacketEntry(connection, ack_packet_seq_number);

  if (packet_entry && !packet_entry->acked) {
    NBN_LogTrace("Packet %d acked (connection: %d)", ack_packet_seq_number,
                 connection->id);
...
static void Connection_InitOutgoingPacket(NBN_Connection *connection,
                                          NBN_Packet *outgoing_packet,
                                          NBN_PacketEntry **packet_entry) {
  NBN_Packet_InitWrite(outgoing_packet, connection->protocol_id,
                       connection->next_packet_seq_number++,
                       connection->last_received_packet_seq_number,
                       Connection_BuildPacketAckBits(connection));

  *packet_entry = Connection_InsertOutgoingPacketEntry(
      connection, outgoing_packet->header.seq_number);
}

static NBN_PacketEntry *
Connection_InsertOutgoingPacketEntry(NBN_Connection *connection,
                                     uint16_t seq_number) {
  uint16_t index = seq_number % NBN_MAX_PACKET_ENTRIES;
  NBN_PacketEntry entry = {.acked = false,
...
static bool Connection_InsertReceivedPacketEntry(NBN_Connection *connection,
                                                 uint16_t seq_number) {
  uint16_t index = seq_number % NBN_MAX_PACKET_ENTRIES;
  ...
  /*
     Clear entries between the previous highest sequence numbers and new highest
     one to avoid entries staying inside the sequence buffer from before the
     sequence wrap around and break the packet acking logic.
     */
  if (SEQUENCE_NUMBER_GT(seq_number,
                         connection->last_received_packet_seq_number)) {
    for (uint16_t seq = connection->last_received_packet_seq_number + 1;
         SEQUENCE_NUMBER_LT(seq, seq_number); seq++)
      connection->packet_recv_seq_buffer[seq % NBN_MAX_PACKET_ENTRIES] =
          0xFFFFFFFF;
  }
...
static NBN_PacketEntry *
Connection_FindSendPacketEntry(NBN_Connection *connection,
                               uint16_t seq_number) {
  uint16_t index = seq_number % NBN_MAX_PACKET_ENTRIES;
...
static bool Connection_IsPacketReceived(NBN_Connection *connection,
...
static int Connection_SendPacket(NBN_Connection *connection, NBN_Packet *packet,
                                 NBN_PacketEntry *packet_entry, double time) {
...
#if defined(NBN_DEBUG) && defined(NBN_USE_PACKET_SIMULATOR)
    return NBN_PacketSimulator_EnqueuePacket(
        &nbn_game_server.endpoint.packet_simulator, packet, connection);
#else
...
#if defined(NBN_DEBUG) && defined(NBN_USE_PACKET_SIMULATOR)
    return NBN_PacketSimulator_EnqueuePacket(
        &nbn_game_client.endpoint.packet_simulator, packet, connection);
#else
...
static int Connection_ReadNextMessageFromStream(NBN_Connection *connection,
                                                NBN_ReadStream *r_stream,
                                                NBN_Message *message) {
  if (NBN_Message_SerializeHeader(&message->header, (NBN_Stream *)r_stream) <
      0) {
  ...
  NBN_MessageBuilder msg_builder =
      connection->endpoint->message_builders[msg_type];

  if (msg_builder == NULL) {
    NBN_LogError("No message builder is registered for messages of type %d",
                 msg_type);
  ...
  NBN_MessageSerializer msg_serializer =
      connection->endpoint->message_serializers[msg_type];

  if (msg_serializer == NULL) {
    NBN_LogError("No message serializer attached to message of type %d",
                 msg_type);
...
static int Connection_ReadNextMessageFromPacket(NBN_Connection *connection,
                                                NBN_Packet *packet,
                                                NBN_Message *message) {
  return Connection_ReadNextMessageFromStream(connection, &packet->r_stream,
                                              message);
}

static void Connection_RecycleMessage(NBN_Channel *channel,
                                      NBN_Message *message) {
  // for incoming messages : message->outgoing_msg == NULL
  assert(message->outgoing_msg == NULL || message->outgoing_msg->ref_count > 0);

  if (message->outgoing_msg == NULL ||
      --message->outgoing_msg->ref_count == 0) {
        ...
        if (--chunk->outgoing_msg->ref_count == 0) {
          NBN_MessageDestructor msg_destructor =
              channel->connection->endpoint
                  ->message_destructors[chunk->outgoing_msg->type];
    ...
    NBN_MessageDestructor msg_destructor =
        channel->connection->endpoint
            ->message_destructors[message->header.type];
...
static void Connection_UpdateAveragePing(NBN_Connection *connection,
                                         double ping) {
  /* exponential smoothing with a factor of 0.05 */
  connection->stats.ping =
      connection->stats.ping + .05f * (ping - connection->stats.ping);
}

static void Connection_UpdateAveragePacketLoss(NBN_Connection *connection,
                                               uint16_t seq) {
  ...
  connection->stats.packet_loss =
      connection->stats.packet_loss +
      .1f * (packet_loss - connection->stats.packet_loss);
}

static void Connection_UpdateAverageUploadBandwidth(NBN_Connection *connection,
                                                    float bytes_per_sec) {
  /* exponential smoothing with a factor of 0.1 */
  connection->stats.upload_bandwidth =
      connection->stats.upload_bandwidth +
      .1f * (bytes_per_sec - connection->stats.upload_bandwidth);
}

static void
Connection_UpdateAverageDownloadBandwidth(NBN_Connection *connection,
                                          double time) {
  ...
  connection->stats.download_bandwidth =
      connection->stats.download_bandwidth +
      .1f * (bytes_per_sec - connection->stats.download_bandwidth);

  connection->downloaded_bytes = 0;
}

static NBN_RPC_Message *Connection_BuildRPC(NBN_Connection *connection,
                                            NBN_RPC *rpc, va_list args) {
  ...
  for (unsigned int i = 0; i < rpc->signature.param_count; i++) {
    ...
    if (param_type == NBN_RPC_PARAM_INT) {
      ...
      NBN_LogError("Calling RPC %d with invalid parameters on connection %d",
                   rpc->id, connection->id);
      NBN_Abort();
...
static void Connection_HandleReceivedRPC(NBN_ConnectionHandle connection,
                                         NBN_Endpoint *endpoint,
                                         NBN_RPC_Message *msg) {
...
bool NBN_Channel_AddChunk(NBN_Channel *channel, NBN_Message *chunk_msg) {
  ...
  NBN_LogTrace("Add chunk %d to channel %d (current chunk count: %d, last "
               "recved chunk id: %d)",
               chunk->id, channel->id, channel->chunk_count,
...
int NBN_Channel_ReconstructMessageFromChunks(NBN_Channel *channel,
                                             NBN_Connection *connection,
                                             NBN_Message *message) {
  ...
  NBN_LogTrace(
      "Reconstructing message (chunk count: %d, size: %d) from channel %d",
      channel->chunk_count, message_size, channel->id);

  for (unsigned int i = 0; i < channel->chunk_count; i++) {
    NBN_MessageChunk *chunk = channel->recv_chunk_buffer[i];

    memcpy(channel->read_chunk_buffer + i * NBN_MESSAGE_CHUNK_SIZE, chunk->data,
  ...
  NBN_LogTrace("Reconstructed message %d of type %d", message->header.id,
               message->header.type);

  return 0;
}

void NBN_Channel_ResizeWriteChunkBuffer(NBN_Channel *channel,
                                        unsigned int size) {
  channel->write_chunk_buffer =
      (uint8_t *)NBN_Reallocator(channel->write_chunk_buffer, size);

  channel->write_chunk_buffer_size = size;
}

void NBN_Channel_ResizeReadChunkBuffer(NBN_Channel *channel,
                                       unsigned int size) {
  channel->read_chunk_buffer =
      (uint8_t *)NBN_Reallocator(channel->read_chunk_buffer, size);

  channel->read_chunk_buffer_size = size;
}

void NBN_Channel_UpdateMessageLastSendTime(NBN_Channel *channel,
                                           NBN_Message *message, double time) {
  NBN_MessageSlot *slot =
      &channel->outgoing_message_slot_buffer[message->header.id %
                                             NBN_CHANNEL_BUFFER_SIZE];
...
static bool UnreliableOrderedChannel_AddReceivedMessage(NBN_Channel *,
                                                        NBN_Message *);
static bool UnreliableOrderedChannel_AddOutgoingMessage(NBN_Channel *,
                                                        NBN_Message *);
static NBN_Message *
UnreliableOrderedChannel_GetNextRecvedMessage(NBN_Channel *);
static NBN_Message *
UnreliableOrderedChannel_GetNextOutgoingMessage(NBN_Channel *, double);
static int UnreliableOrderedChannel_OnMessageSent(NBN_Channel *, NBN_Message *);

NBN_UnreliableOrderedChannel *NBN_UnreliableOrderedChannel_Create(void) {
  NBN_UnreliableOrderedChannel *channel =
      (NBN_UnreliableOrderedChannel *)NBN_Allocator(
          sizeof(NBN_UnreliableOrderedChannel));

  channel->base.next_outgoing_message_pool_slot = 0;
  channel->base.AddReceivedMessage =
      UnreliableOrderedChannel_AddReceivedMessage;
  channel->base.AddOutgoingMessage =
      UnreliableOrderedChannel_AddOutgoingMessage;
  channel->base.GetNextRecvedMessage =
      UnreliableOrderedChannel_GetNextRecvedMessage;
  channel->base.GetNextOutgoingMessage =
      UnreliableOrderedChannel_GetNextOutgoingMessage;
  channel->base.OnOutgoingMessageAcked = NULL;
  channel->base.OnOutgoingMessageSent = UnreliableOrderedChannel_OnMessageSent;

  memset(channel->base.outgoing_message_pool, 0,
         sizeof(channel->base.outgoing_message_pool));
...
static bool UnreliableOrderedChannel_AddReceivedMessage(NBN_Channel *channel,
                                                        NBN_Message *message) {
  NBN_UnreliableOrderedChannel *unreliable_ordered_channel =
      (NBN_UnreliableOrderedChannel *)channel;

  if (SEQUENCE_NUMBER_GT(
          message->header.id,
          unreliable_ordered_channel->last_received_message_id)) {
    NBN_MessageSlot *slot =
        &channel->recved_message_slot_buffer[message->header.id %
                                             NBN_CHANNEL_BUFFER_SIZE];
...
static bool UnreliableOrderedChannel_AddOutgoingMessage(NBN_Channel *channel,
                                                        NBN_Message *message) {
  uint16_t msg_id = channel->next_outgoing_message_id;
  NBN_MessageSlot *slot =
      &channel->outgoing_message_slot_buffer[msg_id % NBN_CHANNEL_BUFFER_SIZE];
...
static NBN_Message *
UnreliableOrderedChannel_GetNextRecvedMessage(NBN_Channel *channel) {
  NBN_UnreliableOrderedChannel *unreliable_ordered_channel =
      (NBN_UnreliableOrderedChannel *)channel;

  while (SEQUENCE_NUMBER_LT(
      channel->next_recv_message_id,
      unreliable_ordered_channel->last_received_message_id)) {
    NBN_MessageSlot *slot =
        &channel->recved_message_slot_buffer[channel->next_recv_message_id %
                                             NBN_CHANNEL_BUFFER_SIZE];

    if (!slot->free &&
        slot->message.header.id == channel->next_recv_message_id) {
      slot->free = true;
...
static NBN_Message *
UnreliableOrderedChannel_GetNextOutgoingMessage(NBN_Channel *channel,
                                                double time) {
  (void)time;
  ...
  NBN_MessageSlot *slot =
      &channel->outgoing_message_slot_buffer[unreliable_ordered_channel
                                                 ->next_outgoing_message_slot];
  ...
  unreliable_ordered_channel->next_outgoing_message_slot =
      (unreliable_ordered_channel->next_outgoing_message_slot + 1) %
      NBN_CHANNEL_BUFFER_SIZE;

  return &slot->message;
}

static int UnreliableOrderedChannel_OnMessageSent(NBN_Channel *channel,
                                                  NBN_Message *message) {
  Connection_RecycleMessage(channel, message);
...
static bool ReliableOrderedChannel_AddReceivedMessage(NBN_Channel *,
                                                      NBN_Message *);
static bool ReliableOrderedChannel_AddOutgoingMessage(NBN_Channel *channel,
                                                      NBN_Message *);
...
static NBN_Message *ReliableOrderedChannel_GetNextOutgoingMessage(NBN_Channel *,
                                                                  double);
static int ReliableOrderedChannel_OnOutgoingMessageAcked(NBN_Channel *,
                                                         uint16_t);

NBN_ReliableOrderedChannel *NBN_ReliableOrderedChannel_Create(void) {
  NBN_ReliableOrderedChannel *channel =
      (NBN_ReliableOrderedChannel *)NBN_Allocator(
          sizeof(NBN_ReliableOrderedChannel));
  ...
  channel->base.GetNextRecvedMessage =
      ReliableOrderedChannel_GetNextRecvedMessage;
  channel->base.GetNextOutgoingMessage =
      ReliableOrderedChannel_GetNextOutgoingMessage;
  channel->base.OnOutgoingMessageAcked =
      ReliableOrderedChannel_OnOutgoingMessageAcked;
  channel->base.OnOutgoingMessageSent = NULL;

  memset(channel->base.recved_message_slot_buffer, 0,
         sizeof(channel->base.recved_message_slot_buffer));
  memset(channel->base.outgoing_message_pool, 0,
         sizeof(channel->base.outgoing_message_pool));
...
static unsigned int ReliableOrderedChannel_ComputeMessageIdDelta(uint16_t id1,
                                                                 uint16_t id2) {
...
static bool ReliableOrderedChannel_AddReceivedMessage(NBN_Channel *channel,
                                                      NBN_Message *message) {
  NBN_ReliableOrderedChannel *reliable_ordered_channel =
      (NBN_ReliableOrderedChannel *)channel;
  ...
  NBN_LogTrace("Add recved message %d of type %d to channel %d (most recent "
               "msg id: %d, dt: %d)",
               message->header.id, message->header.type, channel->id,
               reliable_ordered_channel->most_recent_message_id, dt);

  if (SEQUENCE_NUMBER_GT(message->header.id,
    ...
    /* This is an old message that has already been received, probably coming
       from an out of order late packet. */
...
static bool ReliableOrderedChannel_AddOutgoingMessage(NBN_Channel *channel,
  ...
  if (!slot->free) {
    NBN_LogTrace("Slot %d is not free on channel %d (slot msg id: %d, outgoing "
                 "message count: %d)",
                 index, channel->id, slot->message.header.id,
                 channel->outgoing_message_count);
...
static NBN_Message *
ReliableOrderedChannel_GetNextRecvedMessage(NBN_Channel *channel) {
  NBN_MessageSlot *slot =
      &channel->recved_message_slot_buffer[channel->next_recv_message_id %
                                           NBN_CHANNEL_BUFFER_SIZE];
...
static NBN_Message *
ReliableOrderedChannel_GetNextOutgoingMessage(NBN_Channel *channel,
                                              double time) {
  NBN_ReliableOrderedChannel *reliable_ordered_channel =
      (NBN_ReliableOrderedChannel *)channel;

  int max_message_id = (reliable_ordered_channel->oldest_unacked_message_id +
                        (NBN_CHANNEL_BUFFER_SIZE - 1)) %
                       (0xFFFF + 1);
  ...
  while (SEQUENCE_NUMBER_LT(msg_id, max_message_id)) {
    NBN_MessageSlot *slot =
        &channel
             ->outgoing_message_slot_buffer[msg_id % NBN_CHANNEL_BUFFER_SIZE];

    if (!slot->free &&
        (slot->last_send_time < 0 ||
         time - slot->last_send_time >= NBN_MESSAGE_RESEND_DELAY)) {
      return &slot->message;
    }
...
static int ReliableOrderedChannel_OnOutgoingMessageAcked(NBN_Channel *channel,
                                                         uint16_t msg_id) {
  NBN_ReliableOrderedChannel *reliable_ordered_channel =
      (NBN_ReliableOrderedChannel *)channel;
  NBN_MessageSlot *slot =
      &reliable_ordered_channel->base
           .outgoing_message_slot_buffer[msg_id % NBN_CHANNEL_BUFFER_SIZE];
  ...
  NBN_LogTrace(
      "Message %d acked on channel %d (buffer index: %d, oldest unacked: %d)",
      msg_id, channel->id, msg_id % NBN_CHANNEL_BUFFER_SIZE,
...
static void Endpoint_RegisterMessageBuilder(NBN_Endpoint *, NBN_MessageBuilder,
                                            uint8_t);
static void Endpoint_RegisterMessageDestructor(NBN_Endpoint *,
                                               NBN_MessageDestructor, uint8_t);
static void Endpoint_RegisterMessageSerializer(NBN_Endpoint *,
                                               NBN_MessageSerializer, uint8_t);
static NBN_Connection *Endpoint_CreateConnection(NBN_Endpoint *, uint32_t,
                                                 uint32_t, int, void *);
static int Endpoint_RegisterRPC(NBN_Endpoint *, unsigned int id,
                                NBN_RPC_Signature, NBN_RPC_Func);
static uint32_t Endpoint_BuildProtocolId(const char *);
static void Endpoint_RegisterChannel(NBN_Endpoint *, uint8_t,
                                     NBN_ChannelBuilder, NBN_ChannelDestructor);
static int Endpoint_ProcessReceivedPacket(NBN_Endpoint *, NBN_Packet *,
                                          NBN_Connection *);
static int Endpoint_EnqueueOutgoingMessage(NBN_Endpoint *, NBN_Connection *,
                                           NBN_OutgoingMessage *, uint8_t);
static int Endpoint_SplitMessageIntoChunks(NBN_Message *, NBN_OutgoingMessage *,
                                           NBN_Channel *, NBN_MessageSerializer,
                                           unsigned int, NBN_MessageChunk **);
...
static void Endpoint_Init(NBN_Endpoint *endpoint, bool is_server) {
  ...
  Endpoint_RegisterChannel(
      endpoint, NBN_CHANNEL_RESERVED_UNRELIABLE,
      (NBN_ChannelBuilder)NBN_UnreliableOrderedChannel_Create,
      (NBN_ChannelDestructor)NBN_Channel_Destroy);
  Endpoint_RegisterChannel(
      endpoint, NBN_CHANNEL_RESERVED_RELIABLE,
      (NBN_ChannelBuilder)NBN_ReliableOrderedChannel_Create,
      (NBN_ChannelDestructor)NBN_Channel_Destroy);
  Endpoint_RegisterChannel(
      endpoint, NBN_CHANNEL_RESERVED_LIBRARY_MESSAGES,
      (NBN_ChannelBuilder)NBN_ReliableOrderedChannel_Create,
      (NBN_ChannelDestructor)NBN_Channel_Destroy);
  ...
  for (int i = 0; i < NBN_MAX_CHANNELS - NBN_LIBRARY_RESERVED_CHANNELS; i++) {
    Endpoint_RegisterChannel(
        endpoint, i, (NBN_ChannelBuilder)NBN_ReliableOrderedChannel_Create,
        (NBN_ChannelDestructor)NBN_Channel_Destroy);
  }

  /* Register NBN_MessageChunk library message */
  Endpoint_RegisterMessageBuilder(endpoint,
                                  (NBN_MessageBuilder)NBN_MessageChunk_Create,
                                  NBN_MESSAGE_CHUNK_TYPE);
  Endpoint_RegisterMessageSerializer(
      endpoint, (NBN_MessageSerializer)NBN_MessageChunk_Serialize,
      NBN_MESSAGE_CHUNK_TYPE);
  Endpoint_RegisterMessageDestructor(
      endpoint, (NBN_MessageDestructor)NBN_MessageChunk_Destroy,
      NBN_MESSAGE_CHUNK_TYPE);

  /* Register NBN_ClientClosedMessage library message */
  Endpoint_RegisterMessageBuilder(
      endpoint, (NBN_MessageBuilder)NBN_ClientClosedMessage_Create,
      NBN_CLIENT_CLOSED_MESSAGE_TYPE);
  Endpoint_RegisterMessageSerializer(
      endpoint, (NBN_MessageSerializer)NBN_ClientClosedMessage_Serialize,
      NBN_CLIENT_CLOSED_MESSAGE_TYPE);
  Endpoint_RegisterMessageDestructor(
      endpoint, (NBN_MessageDestructor)NBN_ClientClosedMessage_Destroy,
      NBN_CLIENT_CLOSED_MESSAGE_TYPE);

  /* Register NBN_ClientAcceptedMessage library message */
  Endpoint_RegisterMessageBuilder(
      endpoint, (NBN_MessageBuilder)NBN_ClientAcceptedMessage_Create,
      NBN_CLIENT_ACCEPTED_MESSAGE_TYPE);
  Endpoint_RegisterMessageSerializer(
      endpoint, (NBN_MessageSerializer)NBN_ClientAcceptedMessage_Serialize,
      NBN_CLIENT_ACCEPTED_MESSAGE_TYPE);
  Endpoint_RegisterMessageDestructor(
      endpoint, (NBN_MessageDestructor)NBN_ClientAcceptedMessage_Destroy,
      NBN_CLIENT_ACCEPTED_MESSAGE_TYPE);

  /* Register NBN_ByteArrayMessage library message */
  Endpoint_RegisterMessageBuilder(
      endpoint, (NBN_MessageBuilder)NBN_ByteArrayMessage_Create,
      NBN_BYTE_ARRAY_MESSAGE_TYPE);
  Endpoint_RegisterMessageSerializer(
      endpoint, (NBN_MessageSerializer)NBN_ByteArrayMessage_Serialize,
      NBN_BYTE_ARRAY_MESSAGE_TYPE);
  Endpoint_RegisterMessageDestructor(
      endpoint, (NBN_MessageDestructor)NBN_ByteArrayMessage_Destroy,
      NBN_BYTE_ARRAY_MESSAGE_TYPE);

  /* Register NBN_DisconnectionMessage library message */
  Endpoint_RegisterMessageBuilder(
      endpoint, (NBN_MessageBuilder)NBN_DisconnectionMessage_Create,
      NBN_DISCONNECTION_MESSAGE_TYPE);
  Endpoint_RegisterMessageSerializer(
      endpoint, (NBN_MessageSerializer)NBN_DisconnectionMessage_Serialize,
      NBN_DISCONNECTION_MESSAGE_TYPE);
  Endpoint_RegisterMessageDestructor(
      endpoint, (NBN_MessageDestructor)NBN_DisconnectionMessage_Destroy,
      NBN_DISCONNECTION_MESSAGE_TYPE);

  /* Register NBN_ConnectionRequestMessage library message */
  Endpoint_RegisterMessageBuilder(
      endpoint, (NBN_MessageBuilder)NBN_ConnectionRequestMessage_Create,
      NBN_CONNECTION_REQUEST_MESSAGE_TYPE);
  Endpoint_RegisterMessageSerializer(
      endpoint, (NBN_MessageSerializer)NBN_ConnectionRequestMessage_Serialize,
      NBN_CONNECTION_REQUEST_MESSAGE_TYPE);
  Endpoint_RegisterMessageDestructor(
      endpoint, (NBN_MessageDestructor)NBN_ConnectionRequestMessage_Destroy,
      NBN_CONNECTION_REQUEST_MESSAGE_TYPE);

  /* Register NBN_RPC_Message library message */
  Endpoint_RegisterMessageBuilder(endpoint,
                                  (NBN_MessageBuilder)NBN_RPC_Message_Create,
                                  NBN_RPC_MESSAGE_TYPE);
  Endpoint_RegisterMessageSerializer(
      endpoint, (NBN_MessageSerializer)NBN_RPC_Message_Serialize,
      NBN_RPC_MESSAGE_TYPE);
  Endpoint_RegisterMessageDestructor(
      endpoint, (NBN_MessageDestructor)NBN_RPC_Message_Destroy,
      NBN_RPC_MESSAGE_TYPE);
...
static void Endpoint_RegisterMessageBuilder(NBN_Endpoint *endpoint,
                                            NBN_MessageBuilder msg_builder,
                                            uint8_t msg_type) {
...
static void
Endpoint_RegisterMessageDestructor(NBN_Endpoint *endpoint,
                                   NBN_MessageDestructor msg_destructor,
                                   uint8_t msg_type) {
...
static void
Endpoint_RegisterMessageSerializer(NBN_Endpoint *endpoint,
                                   NBN_MessageSerializer msg_serializer,
                                   uint8_t msg_type) {
  endpoint->message_serializers[msg_type] = msg_serializer;
}

static NBN_Connection *Endpoint_CreateConnection(NBN_Endpoint *endpoint,
                                                 uint32_t id,
                                                 uint32_t protocol_id,
                                                 int driver_id,
                                                 void *driver_data) {
  ...
  NBN_Connection *connection =
      NBN_Connection_Create(id, protocol_id, endpoint, driver, driver_data);
...
static int Endpoint_RegisterRPC(NBN_Endpoint *endpoint, unsigned int id,
                                NBN_RPC_Signature signature,
                                NBN_RPC_Func func) {
  if (id >= NBN_RPC_MAX) {
  ...
  NBN_LogDebug("Registered RPC (id: %d, parameter count: %d)", id,
...
static void Endpoint_RegisterChannel(NBN_Endpoint *endpoint, uint8_t id,
                                     NBN_ChannelBuilder builder,
                                     NBN_ChannelDestructor destructor) {
...
static int Endpoint_ProcessReceivedPacket(NBN_Endpoint *endpoint,
                                          NBN_Packet *packet,
                                          NBN_Connection *connection) {
  (void)endpoint;

  NBN_LogTrace("Received packet %d (conn id: %d, ack: %d, messages count: %d)",
               packet->header.seq_number, connection->id, packet->header.ack,
               packet->header.messages_count);

  if (NBN_Connection_ProcessReceivedPacket(connection, packet, endpoint->time) <
      0)
    return NBN_ERROR;
...
static NBN_OutgoingMessage *
Endpoint_CreateOutgoingMessage(NBN_Endpoint *endpoint, NBN_Channel *channel,
                               uint8_t msg_type, void *data) {
  assert(channel);

  if (endpoint->message_serializers[msg_type] == NULL) {
    NBN_LogError("No message serializer is registered for messages of type %d",
                 msg_type);
    NBN_Abort();
  }

  NBN_OutgoingMessage *outgoing_message =
      &channel->outgoing_message_pool[channel->next_outgoing_message_pool_slot];
  ...
  channel->next_outgoing_message_pool_slot =
      (channel->next_outgoing_message_pool_slot + 1) %
      NBN_CHANNEL_OUTGOING_MESSAGE_POOL_SIZE;

  return outgoing_message;
}

static int Endpoint_EnqueueOutgoingMessage(NBN_Endpoint *endpoint,
                                           NBN_Connection *connection,
                                           NBN_OutgoingMessage *outgoing_msg,
                                           uint8_t channel_id) {
  ...
  NBN_MessageSerializer msg_serializer =
      endpoint->message_serializers[outgoing_msg->type];

  assert(msg_serializer);

  NBN_Message message = {{0, outgoing_msg->type, channel_id},
                         NULL,
                         outgoing_msg,
                         outgoing_msg->data};
  ...
  unsigned int message_size =
      (NBN_Message_Measure(&message, &m_stream, msg_serializer) - 1) / 8 + 1;
  ...
  if (message_size > NBN_PACKET_MAX_DATA_SIZE) {
    ...
    for (int i = 0; i < chunk_count; i++) {
      ...
      if (Endpoint_EnqueueOutgoingMessage(endpoint, connection,
                                          chunk_outgoing_msg, channel_id) < 0) {
...
static int Endpoint_SplitMessageIntoChunks(NBN_Message *message,
                                           NBN_OutgoingMessage *outgoing_msg,
                                           NBN_Channel *channel,
                                           NBN_MessageSerializer msg_serializer,
                                           unsigned int message_size,
  ...
  NBN_LogTrace("Split message into %d chunks (message size: %d)", chunk_count,
               message_size);
  ...
  if (NBN_Message_SerializeHeader(&message->header, (NBN_Stream *)&w_stream) <
      0)
    return NBN_ERROR;

  if (NBN_Message_SerializeData(message, (NBN_Stream *)&w_stream,
                                msg_serializer) < 0)
    return NBN_ERROR;
  ...
  for (unsigned int i = 0; i < chunk_count; i++) {
    ...
    unsigned int chunk_size =
        MIN(NBN_MESSAGE_CHUNK_SIZE, message_size - offset);
    ...
    NBN_LogTrace(
        "Enqueue chunk %d (size: %d, total: %d) for message %d of type %d",
        chunk->id, chunk_size, chunk->total, message->header.id,
        message->header.type);
...
int NBN_GameClient_StartEx(const char *protocol_name, const char *host,
  ...
  if (data && length > NBN_CONNECTION_DATA_MAX_SIZE) {
    NBN_LogError("Connection data cannot exceed %d bytes",
  ...
  for (unsigned int i = 0; i < NBN_MAX_DRIVERS; i++) {
    ...
    if (driver->impl.cli_start(Endpoint_BuildProtocolId(protocol_name), host,
                               port) < 0) {
  ...
  if (NBN_GameClient_SendMessage(NBN_CONNECTION_REQUEST_MESSAGE_TYPE,
...
void NBN_GameClient_Stop(void) {
  ...
  if (nbn_game_client.server_connection) {
    if (!nbn_game_client.server_connection->is_closed &&
        !nbn_game_client.server_connection->is_stale) {
      NBN_LogInfo("Disconnecting...");

      if (NBN_GameClient_SendMessage(NBN_DISCONNECTION_MESSAGE_TYPE,
                                     NBN_CHANNEL_RESERVED_LIBRARY_MESSAGES,
                                     NULL) < 0) {
...
void NBN_GameClient_RegisterMessage(uint8_t msg_type,
  ...
  Endpoint_RegisterMessageBuilder(&nbn_game_client.endpoint, msg_builder,
                                  msg_type);
  Endpoint_RegisterMessageDestructor(&nbn_game_client.endpoint, msg_destructor,
                                     msg_type);
  Endpoint_RegisterMessageSerializer(&nbn_game_client.endpoint, msg_serializer,
                                     msg_type);
}
...
int NBN_GameClient_Poll(void) {
  ...
  if (NBN_EventQueue_IsEmpty(&nbn_game_client.endpoint.event_queue)) {
    if (NBN_Connection_CheckIfStale(nbn_game_client.server_connection,
      ...
      for (unsigned int i = 0; i < NBN_MAX_CHANNELS; i++) {
        ...
        if (channel) {
          ...
          while ((msg = channel->GetNextRecvedMessage(channel)) != NULL) {
            NBN_LogTrace("Got message %d of type %d from the recv queue",
                         msg->header.id, msg->header.type);

            if (GameClient_ProcessReceivedMessage(
                    msg, nbn_game_client.server_connection) < 0) {
      ...
      Connection_UpdateAverageDownloadBandwidth(
          nbn_game_client.server_connection, nbn_game_client.endpoint.time);

      nbn_game_client.server_connection->last_read_packets_time =
          nbn_game_client.endpoint.time;
    }
  }

  bool ret = NBN_EventQueue_Dequeue(&nbn_game_client.endpoint.event_queue,
                                    &nbn_game_client.last_event);
...
int NBN_GameClient_SendMessage(uint8_t msg_type, uint8_t channel_id,
  ...
  NBN_OutgoingMessage *outgoing_msg = Endpoint_CreateOutgoingMessage(
      ...
      nbn_game_client.server_connection->channels[channel_id], msg_type,
      msg_data);
  ...
  if (Endpoint_EnqueueOutgoingMessage(&nbn_game_client.endpoint,
                                      nbn_game_client.server_connection,
                                      outgoing_msg, channel_id) < 0) {
    NBN_LogError("Failed to create outgoing message");
...
int NBN_GameClient_SendByteArray(uint8_t *bytes, unsigned int length,
  ...
  return NBN_GameClient_SendMessage(NBN_BYTE_ARRAY_MESSAGE_TYPE, channel_id,
                                    msg);
}

int NBN_GameClient_SendUnreliableMessage(uint8_t msg_type, void *msg_data) {
  return NBN_GameClient_SendMessage(msg_type, NBN_CHANNEL_RESERVED_UNRELIABLE,
                                    msg_data);
}

int NBN_GameClient_SendReliableMessage(uint8_t msg_type, void *msg_data) {
  return NBN_GameClient_SendMessage(msg_type, NBN_CHANNEL_RESERVED_RELIABLE,
                                    msg_data);
}

int NBN_GameClient_SendUnreliableByteArray(uint8_t *bytes,
                                           unsigned int length) {
  return NBN_GameClient_SendByteArray(bytes, length,
...
NBN_Connection *NBN_GameClient_CreateServerConnection(int driver_id,
                                                      void *driver_data,
                                                      uint32_t protocol_id) {
  NBN_Connection *server_connection = Endpoint_CreateConnection(
      &nbn_game_client.endpoint, 0, protocol_id, driver_id, driver_data);
...
int NBN_GameClient_CallRPC(unsigned int id, ...) {
  ...
  NBN_RPC_Message *rpc_msg =
      Connection_BuildRPC(nbn_game_client.server_connection, &rpc, args);

  assert(rpc_msg);

  if (NBN_GameClient_SendMessage(NBN_RPC_MESSAGE_TYPE,
                                 NBN_CHANNEL_RESERVED_LIBRARY_MESSAGES,
                                 rpc_msg) < 0) {
...
static int
GameClient_ProcessReceivedMessage(NBN_Message *message,
                                  NBN_Connection *server_connection) {
  ...
  if (message->header.type == NBN_MESSAGE_CHUNK_TYPE) {
    NBN_Channel *channel =
        server_connection->channels[message->header.channel_id];
    ...
    if (NBN_Channel_ReconstructMessageFromChunks(channel, server_connection,
                                                 &complete_message) < 0) {
    ...
    NBN_MessageInfo msg_info = {complete_message.header.type,
                                complete_message.header.channel_id,
                                complete_message.data, 0};

    ev.data.message_info = msg_info;
  } else {
    NBN_MessageInfo msg_info = {message->header.type,
                                message->header.channel_id, message->data, 0};
...
static int GameClient_HandleMessageReceivedEvent(void) {
  ...
  if (message_info.type == NBN_CLIENT_CLOSED_MESSAGE_TYPE) {
    nbn_game_client.is_connected = false;
    nbn_game_client.closed_code =
        ((NBN_ClientClosedMessage *)message_info.data)->code;
    ret = NBN_DISCONNECTED;
  } else if (message_info.type == NBN_CLIENT_ACCEPTED_MESSAGE_TYPE) {
    nbn_game_client.is_connected = true;
    ret = NBN_CONNECTED;

    NBN_ClientAcceptedMessage *accepted_msg =
        (NBN_ClientAcceptedMessage *)message_info.data;

    memcpy(nbn_game_client.server_data, accepted_msg->data,
           accepted_msg->length);
    nbn_game_client.server_data_len = accepted_msg->length;
  } else if (message_info.type == NBN_RPC_MESSAGE_TYPE) {
    ret = NBN_NO_EVENT;

    Connection_HandleReceivedRPC(nbn_game_client.server_connection->id,
                                 &nbn_game_client.endpoint,
                                 (NBN_RPC_Message *)message_info.data);
  } else {
...
static void ClientDriver_OnPacketReceived(NBN_Packet *packet) {
  ...
  if (Endpoint_ProcessReceivedPacket(&nbn_game_client.endpoint, packet,
                                     nbn_game_client.server_connection) < 0) {
...
static int GameServer_SendMessageTo(NBN_Connection *client, uint8_t msg_type,
                                    uint8_t channel_id, void *msg_data);
static int GameServer_AddClient(NBN_Connection *);
static int GameServer_CloseClientWithCode(NBN_Connection *client, int code,
                                          bool disconnection);
...
int NBN_GameServer_StartEx(const char *protocol_name, uint16_t port) {
  ...
  for (unsigned int i = 0; i < NBN_MAX_DRIVERS; i++) {
    ...
    if (driver->impl.serv_start(Endpoint_BuildProtocolId(protocol_name), port) <
        0) {
      NBN_LogError("Failed to start driver %s", driver->name);
      return NBN_ERROR;
    }
...
void NBN_GameServer_RegisterMessage(uint8_t msg_type,
  ...
  Endpoint_RegisterMessageBuilder(&nbn_game_server.endpoint, msg_builder,
                                  msg_type);
  Endpoint_RegisterMessageDestructor(&nbn_game_server.endpoint, msg_destructor,
                                     msg_type);
  Endpoint_RegisterMessageSerializer(&nbn_game_server.endpoint, msg_serializer,
                                     msg_type);
}
...
int NBN_GameServer_Poll(void) {
  ...
  if (NBN_EventQueue_IsEmpty(&nbn_game_server.endpoint.event_queue)) {
    ...
    for (unsigned int i = 0; i < nbn_game_server.clients->count; i++) {
      ...
      if (!client->is_closed)
        Connection_UpdateAverageDownloadBandwidth(
            client, nbn_game_server.endpoint.time);

      nbn_game_server.stats.download_bandwidth +=
          client->stats.download_bandwidth;
  ...
  while (NBN_EventQueue_Dequeue(&nbn_game_server.endpoint.event_queue,
                                &nbn_game_server.last_event)) {
...
int NBN_GameServer_SendPackets(void) {
  ...
  for (unsigned int i = 0; i < nbn_game_server.clients->count; i++) {
    ...
    if (!client->is_stale && NBN_Connection_FlushSendQueue(
                                 client, nbn_game_server.endpoint.time) < 0)
      return NBN_ERROR;
...
NBN_Connection *NBN_GameServer_CreateClientConnection(int driver_id,
                                                      void *driver_data,
                                                      uint32_t protocol_id,
                                                      uint32_t conn_id) {
  ...
  NBN_Connection *client = Endpoint_CreateConnection(
      &nbn_game_server.endpoint, conn_id, protocol_id, driver_id, driver_data);
...
int NBN_GameServer_CloseClientWithCode(NBN_ConnectionHandle connection_handle,
  ...
  NBN_Connection *client =
      NBN_ConnectionTable_Get(nbn_game_server.clients_table, connection_handle);
...
int NBN_GameServer_SendByteArrayTo(NBN_ConnectionHandle connection_handle,
  ...
  return NBN_GameServer_SendMessageTo(
      connection_handle, NBN_BYTE_ARRAY_MESSAGE_TYPE, channel_id, msg);
}
...
int NBN_GameServer_SendMessageTo(NBN_ConnectionHandle connection_handle,
  ...
  if (client == NULL) {
    NBN_LogWarning("Cannot send message to client %d (does not exist)",
                   connection_handle);
    return 0;
  }
...
int NBN_GameServer_SendUnreliableMessageTo(
    NBN_ConnectionHandle connection_handle, uint8_t msg_type, void *msg_data) {
  return NBN_GameServer_SendMessageTo(
      connection_handle, msg_type, NBN_CHANNEL_RESERVED_UNRELIABLE, msg_data);
}

int NBN_GameServer_SendReliableMessageTo(NBN_ConnectionHandle connection_handle,
                                         uint8_t msg_type, void *msg_data) {
  return NBN_GameServer_SendMessageTo(connection_handle, msg_type,
                                      NBN_CHANNEL_RESERVED_RELIABLE, msg_data);
}

int NBN_GameServer_SendUnreliableByteArrayTo(
    NBN_ConnectionHandle connection_handle, uint8_t *bytes,
    unsigned int length) {
  return NBN_GameServer_SendByteArrayTo(connection_handle, bytes, length,
                                        NBN_CHANNEL_RESERVED_UNRELIABLE);
}
...
int NBN_GameServer_AcceptIncomingConnectionWithData(uint8_t *data,
  ...
  if (length > NBN_SERVER_DATA_MAX_SIZE) {
    NBN_LogError("Data length cannot exceed %d bytes",
                 NBN_SERVER_DATA_MAX_SIZE);
    return NBN_ERROR;
  }
  ...
  if (GameServer_SendMessageTo(client, NBN_CLIENT_ACCEPTED_MESSAGE_TYPE,
                               NBN_CHANNEL_RESERVED_LIBRARY_MESSAGES, msg) < 0)
    return NBN_ERROR;
...
int NBN_GameServer_RejectIncomingConnectionWithCode(int code) {
  ...
  return GameServer_CloseClientWithCode(
      nbn_game_server.last_event.data.connection, code, false);
}
...
unsigned int NBN_GameServer_ReadIncomingConnectionData(uint8_t *data) {
  ...
  memcpy(data, nbn_game_server.last_connection_data,
         nbn_game_server.last_connection_data_len);
...
int NBN_GameServer_CallRPC(unsigned int id,
  ...
  if (GameServer_SendMessageTo(client, NBN_RPC_MESSAGE_TYPE,
                               NBN_CHANNEL_RESERVED_LIBRARY_MESSAGES,
                               rpc_msg) < 0) {
    NBN_LogError("Failed to send RPC message");
...
static int GameServer_SendMessageTo(NBN_Connection *client, uint8_t msg_type,
                                    uint8_t channel_id, void *msg_data) {
  NBN_OutgoingMessage *outgoing_msg = Endpoint_CreateOutgoingMessage(
      &nbn_game_server.endpoint, client->channels[channel_id], msg_type,
      msg_data);
  ...
  /* The only message type we can send to an unaccepted client is a
   * NBN_ClientAcceptedMessage message */
  assert(client->is_accepted ||
         outgoing_msg->type == NBN_CLIENT_ACCEPTED_MESSAGE_TYPE ||
         outgoing_msg->type == NBN_CLIENT_CLOSED_MESSAGE_TYPE);

  if (Endpoint_EnqueueOutgoingMessage(&nbn_game_server.endpoint, client,
                                      outgoing_msg, channel_id) < 0) {
    NBN_LogError("Failed to create outgoing message for client %d", client->id);

    /* Do not close the client if we failed to send the close client message to
...
static int GameServer_CloseClientWithCode(NBN_Connection *client, int code,
  ...
  NBN_LogDebug(
      "Closing active connection %d (will send a disconnection message)",
      client->id);
  ...
  if (!disconnection) {
    NBN_LogDebug("Send close message for client %d (code: %d)", client->id,
                 code);
    ...
    GameServer_SendMessageTo(client, NBN_CLIENT_CLOSED_MESSAGE_TYPE,
                             NBN_CHANNEL_RESERVED_LIBRARY_MESSAGES, msg);
  }
...
static void GameServer_AddClientToClosedList(NBN_Connection *client) {
  ...
  NBN_ConnectionListNode *node =
      (NBN_ConnectionListNode *)NBN_Allocator(sizeof(NBN_ConnectionListNode));
...
static int GameServer_ProcessReceivedMessage(NBN_Message *message,
                                             NBN_Connection *client) {
  NBN_Event ev;
  ...
  if (message->header.type == NBN_MESSAGE_CHUNK_TYPE) {
    ...
    if (NBN_Channel_ReconstructMessageFromChunks(channel, client,
                                                 &complete_message) < 0) {
      NBN_LogError("Failed to reconstruct message from chunks");
    ...
    NBN_MessageInfo msg_info = {complete_message.header.type,
                                complete_message.header.channel_id,
                                complete_message.data, client->id};

    ev.data.message_info = msg_info;
  } else {
    NBN_MessageInfo msg_info = {message->header.type,
                                message->header.channel_id, message->data,
                                client->id};
...
static int GameServer_CloseStaleClientConnections(void) {
  for (unsigned int i = 0; i < nbn_game_server.clients->count; i++) {
    ...
    if (!client->is_stale &&
        NBN_Connection_CheckIfStale(client, nbn_game_server.endpoint.time)) {
...
static void GameServer_RemoveClosedClientConnections(void) {
  ...
  while (current) {
    ...
    if (client->is_stale) {
      ...
      client->driver->impl.serv_remove_connection(
          client); // Notify the driver to remove the connection

      // Remove the connection from the connections vector and table

      uint32_t rm_conn_id = NBN_ConnectionVector_RemoveAt(
          nbn_game_server.clients, client->vector_pos);

      if (rm_conn_id != client->id) {
        NBN_LogError(
            "Failed to remove client connection from connections vector");
        NBN_Abort();
      }

      bool table_rm =
          NBN_ConnectionTable_Remove(nbn_game_server.clients_table, client->id);

      if (!table_rm) {
        NBN_LogError(
            "Failed to remove client connection from connections table");
        NBN_Abort();
      }
...
static int GameServer_HandleMessageReceivedEvent(void) {
  ...
  NBN_Connection *sender = NBN_ConnectionTable_Get(
      nbn_game_server.clients_table, message_info.sender);

  if (sender == NULL) {
    // skip events related to unknown clients (clients that might have been
    // removed)
    NBN_LogTrace("Received message from unknown client (ID: %d)",
                 message_info.sender);
  ...
  if (message_info.type == NBN_RPC_MESSAGE_TYPE) {
    ...
    Connection_HandleReceivedRPC(message_info.sender, &nbn_game_server.endpoint,
                                 (NBN_RPC_Message *)message_info.data);
  } else if (message_info.type == NBN_CONNECTION_REQUEST_MESSAGE_TYPE) {
    ret = NBN_NO_EVENT;

    NBN_ConnectionRequestMessage *msg =
        (NBN_ConnectionRequestMessage *)message_info.data;
...
static int ServerDriver_OnClientPacketReceived(NBN_Packet *packet) {
  if (Endpoint_ProcessReceivedPacket(&nbn_game_server.endpoint, packet,
                                     packet->sender) < 0) {
    NBN_LogError("An error occured while processing packet from client %d, "
                 "closing the client",
                 packet->sender->id);
...
#define RAND_RATIO_BETWEEN(min, max)                                           \
  (((rand() % (int)((max * 100.f) - (min * 100.f) + 1)) + (min * 100.f)) /     \
   100.f)
...
static int PacketSimulator_SendPacket(NBN_PacketSimulator *, NBN_Packet *,
                                      NBN_Connection *receiver);
static unsigned int
PacketSimulator_GetRandomDuplicatePacketCount(NBN_PacketSimulator *);
...
int NBN_PacketSimulator_EnqueuePacket(NBN_PacketSimulator *packet_simulator,
                                      NBN_Packet *packet,
                                      NBN_Connection *receiver) {
#ifdef NBNET_WINDOWS
   ...
   * Jitter is converted from seconds to milliseconds for the random operation
   * below.
   */
  ...
  NBN_PacketSimulatorEntry *entry =
      (NBN_PacketSimulatorEntry *)MemoryManager_Alloc(
          NBN_MEM_PACKET_SIMULATOR_ENTRY);

  entry->delay = packet_simulator->ping +
                 (double)jitter / 1000; /* and converted back to seconds */
...
#ifdef NBNET_WINDOWS
  packet_simulator->thread =
      CreateThread(NULL, 0, PacketSimulator_Routine, packet_simulator, 0, NULL);
#else
  pthread_create(&packet_simulator->thread, NULL, PacketSimulator_Routine,
                 packet_simulator);
#endif
...
#endif
{
  ...
  while (packet_simulator->running) {
    ...
    while (entry) {
      ...
      if (packet_simulator->endpoint->time - entry->enqueued_at <
          entry->delay) {
        entry = next;
      ...
      PacketSimulator_SendPacket(packet_simulator, &entry->packet,
                                 entry->receiver);

      for (unsigned int i = 0;
           i < PacketSimulator_GetRandomDuplicatePacketCount(packet_simulator);
           i++) {
        NBN_LogDebug("Duplicate packet %d (count: %d)",
                     entry->packet.header.seq_number, i + 1);
      ...
      if (entry == packet_simulator->head_packet) // it's the head of the list
      {
        ...
        packet_simulator->head_packet = new_head;
                 ...
                 packet_simulator->tail_packet) // it's the tail of the list
      {
...
static int PacketSimulator_SendPacket(NBN_PacketSimulator *packet_simulator,
                                      NBN_Packet *packet,
                                      NBN_Connection *receiver) {
  if (RAND_RATIO < packet_simulator->packet_loss_ratio) {
    packet_simulator->total_dropped_packets++;
    NBN_LogDebug("Drop packet %d (Total dropped packets: %d)",
                 packet->header.seq_number,
                 packet_simulator->total_dropped_packets);
...
static unsigned int PacketSimulator_GetRandomDuplicatePacketCount(
    NBN_PacketSimulator *packet_simulator) {
