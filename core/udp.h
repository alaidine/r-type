...
/*
    ...
    How to use:
        ...
        1. Include this header *once* after the nbnet header in the same file
   where you defined the NBNET_IMPL macro
        2. Call NBN_UDP_Register in both your client and server code before
   calling NBN_GameClient_Start or NBN_GameServer_Start
*/
...
static void NBN_UDP_HTable_Add(NBN_UDP_HTable *, NBN_IPAddress,
                               NBN_UDP_Connection *);
static NBN_UDP_Connection *NBN_UDP_HTable_Get(NBN_UDP_HTable *, NBN_IPAddress);
static NBN_UDP_Connection *NBN_UDP_HTable_Remove(NBN_UDP_HTable *,
                                                 NBN_IPAddress);
...
static unsigned int NBN_UDP_HTable_FindFreeSlot(NBN_UDP_HTable *,
                                                NBN_UDP_HTableEntry *, bool *);
static NBN_UDP_HTableEntry *NBN_UDP_HTable_FindEntry(NBN_UDP_HTable *,
                                                     NBN_IPAddress);
...
static NBN_UDP_HTable *
NBN_UDP_HTable_CreateWithCapacity(unsigned int capacity) {
  NBN_UDP_HTable *htable =
      (NBN_UDP_HTable *)NBN_Allocator(sizeof(NBN_UDP_HTable));

  htable->internal_array = (NBN_UDP_HTableEntry **)NBN_Allocator(
      sizeof(NBN_UDP_HTableEntry *) * capacity);
...
static void NBN_UDP_HTable_Add(NBN_UDP_HTable *htable, NBN_IPAddress ip_addr,
                               NBN_UDP_Connection *conn) {
  NBN_UDP_HTableEntry *entry =
      (NBN_UDP_HTableEntry *)NBN_Allocator(sizeof(NBN_UDP_HTableEntry));
...
static NBN_UDP_Connection *NBN_UDP_HTable_Get(NBN_UDP_HTable *htable,
                                              NBN_IPAddress ip_addr) {
...
static NBN_UDP_Connection *NBN_UDP_HTable_Remove(NBN_UDP_HTable *htable,
                                                 NBN_IPAddress ip_addr) {
  NBN_UDP_HTableEntry *entry = NBN_UDP_HTable_FindEntry(htable, ip_addr);
...
static void NBN_UDP_HTable_InsertEntry(NBN_UDP_HTable *htable,
                                       NBN_UDP_HTableEntry *entry) {
  bool use_existing_slot = false;
  unsigned int slot =
      NBN_UDP_HTable_FindFreeSlot(htable, entry, &use_existing_slot);
...
static void NBN_UDP_HTable_RemoveEntry(NBN_UDP_HTable *htable,
                                       NBN_UDP_HTableEntry *entry) {
...
static unsigned int NBN_UDP_HTable_FindFreeSlot(NBN_UDP_HTable *htable,
                                                NBN_UDP_HTableEntry *entry,
  ...
  do {
    ...
    i++;
           ...
           !CompareIPAddresses(current_entry->ip_addr, entry->ip_addr));

  if (current_entry !=
      NULL) // it means the current entry as the same key as the inserted entry
  {
...
static NBN_UDP_HTableEntry *NBN_UDP_HTable_FindEntry(NBN_UDP_HTable *htable,
                                                     NBN_IPAddress ip_addr) {
  ...
  do {
    ...
    if (current_entry != NULL &&
        CompareIPAddresses(current_entry->ip_addr, ip_addr)) {
...
static int ResolveIpAddress(const char *host, uint16_t port,
                            NBN_IPAddress *address) {
...
static int NBN_UDP_ServRecvPackets(void) {
  ...
  while (true) {
    int bytes = recvfrom(nbn_udp_sock, (char *)buffer, sizeof(buffer), 0,
    ...
    if (NBN_Driver_RaiseEvent(NBN_DRIVER_SERV_CLIENT_PACKET_RECEIVED, &packet) <
        0) {
...
static void NBN_UDP_ServRemoveClientConnection(NBN_Connection *connection) {
  ...
  NBN_UDP_Connection *udp_conn = NBN_UDP_HTable_Remove(
      nbn_udp_serv.connections,
      ((NBN_UDP_Connection *)connection->driver_data)->address);
...
static int NBN_UDP_ServSendPacketTo(NBN_Packet *packet,
  ...
  if (sendto(nbn_udp_sock, (const char *)packet->buffer, packet->size, 0,
...
static NBN_Connection *
FindOrCreateClientConnectionByAddress(NBN_IPAddress address) {
  NBN_UDP_Connection *udp_conn =
      NBN_UDP_HTable_Get(nbn_udp_serv.connections, address);
  ...
  if (udp_conn == NULL) {
    ...
    udp_conn->conn = NBN_GameServer_CreateClientConnection(
        NBN_UDP_DRIVER_ID, udp_conn, nbn_udp_serv.protocol_id, udp_conn->id);

    NBN_UDP_HTable_Add(nbn_udp_serv.connections, address, udp_conn);

    NBN_LogDebug("New UDP connection (id: %d)", udp_conn->id);

    if (NBN_Driver_RaiseEvent(NBN_DRIVER_SERV_CLIENT_CONNECTED,
                              udp_conn->conn) < 0) {
      NBN_LogError("Failed to raise game server event");
...
static int NBN_UDP_CliStart(uint32_t protocol_id, const char *host,
                            uint16_t port) {
  NBN_UDP_Connection *udp_conn =
      (NBN_UDP_Connection *)NBN_Allocator(sizeof(NBN_Connection));
  ...
  nbn_udp_cli.server_conn = NBN_GameClient_CreateServerConnection(
      NBN_UDP_DRIVER_ID, udp_conn, protocol_id);
...
static void NBN_UDP_CliStop(void) {
  NBN_UDP_Connection *udp_conn =
      (NBN_UDP_Connection *)nbn_udp_cli.server_conn->driver_data;
...
static int NBN_UDP_CliRecvPackets(void) {
  ...
  while (true) {
    ...
    if (ip_address.host != udp_conn->address.host ||
        ip_address.port != udp_conn->address.port)
      continue;
    ...
    if (NBN_Packet_InitRead(&packet, nbn_udp_cli.server_conn, buffer, bytes) <
        0)
      continue; /* not a valid packet */
