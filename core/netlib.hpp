// netlib.hpp - Simple UDP networking library with serialization
#ifndef NETLIB_HPP
#define NETLIB_HPP
...
public:
    NetBuffer(size_t capacity = 4096) 
        : _size(0), _capacity(capacity), _readPos(0) {
    ...
    void WriteUInt8(uint8_t value) {
        if (_size + sizeof(uint8_t) > _capacity) return;
        _data[_size++] = value;
    }

    void WriteUInt16(uint16_t value) {
        if (_size + sizeof(uint16_t) > _capacity) return;
        memcpy(_data + _size, &value, sizeof(uint16_t));
        _size += sizeof(uint16_t);
    }

    void WriteUInt32(uint32_t value) {
        if (_size + sizeof(uint32_t) > _capacity) return;
        memcpy(_data + _size, &value, sizeof(uint32_t));
        _size += sizeof(uint32_t);
    }

    void WriteInt32(int32_t value) {
        if (_size + sizeof(int32_t) > _capacity) return;
        memcpy(_data + _size, &value, sizeof(int32_t));
        _size += sizeof(int32_t);
    }

    void WriteFloat(float value) {
        if (_size + sizeof(float) > _capacity) return;
    ...
    // Read operations
    uint8_t ReadUInt8() {
        if (_readPos + sizeof(uint8_t) > _size) return 0;
        return _data[_readPos++];
    }

    uint16_t ReadUInt16() {
        if (_readPos + sizeof(uint16_t) > _size) return 0;
        uint16_t value;
    ...
    uint32_t ReadUInt32() {
        if (_readPos + sizeof(uint32_t) > _size) return 0;
        uint32_t value;
    ...
    int32_t ReadInt32() {
        if (_readPos + sizeof(int32_t) > _size) return 0;
        int32_t value;
    ...
    float ReadFloat() {
        if (_readPos + sizeof(float) > _size) return 0.0f;
        float value;
    ...
    void ReadBytes(void* dest, size_t length) {
        if (_readPos + length > _size) return;
...
public:
    ...
    bool SendTo(const NetBuffer& buffer, const sockaddr_in& dest) {
        int sent = sendto(_sockfd, (const char*)buffer.GetData(), (int)buffer.GetSize(), 0,
    ...
    int ReceiveFrom(NetBuffer& buffer, sockaddr_in& from) {
        ...
        int received = recvfrom(_sockfd, temp, sizeof(temp), 0,
...
public:
    ...
    bool Start() {
        return _socket.Bind(_port);
    }

    int Receive(NetBuffer& buffer, sockaddr_in& from) {
        return _socket.ReceiveFrom(buffer, from);
    }

    bool SendTo(const NetBuffer& buffer, const sockaddr_in& dest) {
        return _socket.SendTo(buffer, dest);
    }
