...
namespace rt {
	...
	std::vector<const char*> Renderer::args;
	...
	*/
	...
	std::string Renderer::getShadersPath() const
	{
	...
	void Renderer::OnUpdateUIOverlay(vks::UIOverlay* overlay)
	{
	...
	void Renderer::prepareFrame(bool waitForFence)
	{
	...
	void Renderer::submitFrame(bool skipQueueSubmit)
	{
		if (!skipQueueSubmit) {
	...
	void Renderer::setupConsole(std::string title)
	{
		AllocConsole();
	...
	void Renderer::setupDPIAwareness()
	{
	...
	HWND Renderer::setupWindow(HINSTANCE hinstance, WNDPROC wndproc)
	{
	...
	void Renderer::keyPressed(uint32_t) {}
	...
	void Renderer::getEnabledFeatures()
	{
	...
	void Renderer::getEnabledExtensions() {}
	...
	void Renderer::handleMouseMove(int32_t x, int32_t y)
	{
	...
	void Renderer::OnHandleMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {}

	// Free all Vulkan resources used by a texture object
	void Renderer::destroyTextureImage(Texture texture)
	{
	...
	void Renderer::DrawTextureRec(Texture texture, Rectangle rectangle, glm::vec2 position, Color tint)
	{
		// Convert position and dimensions to normalized device coordinates (NDC)
		float ndc_x = (position.x / width) * 2.0f - 1.0f;
		float ndc_y = 1.0f - (position.y / height) * 2.0f;
		float ndc_width = (rectangle.width / width) * 2.0f;
		float ndc_height = (rectangle.height / height) * 2.0f;

		// Calculate UVs from rectangle (sub-region of the texture)
		float u0 = rectangle.x / static_cast<float>(texture.width);
		float v0 = rectangle.y / static_cast<float>(texture.height);
		float u1 = (rectangle.x + rectangle.width) / static_cast<float>(texture.width);
		float v1 = (rectangle.y + rectangle.height) / static_cast<float>(texture.height);

		// Vulkan and most image formats have (0,0) at top-left, but UVs are usually bottom-left.
		// If the images appear vertically flipped, swap v0 and v1.
		// Here, v0 is top, v1 is bottom (KTX is usually bottom-left origin, so swap if needed).
	...
	void Renderer::EndDrawing()
	{
		if (prepared && !IsIconic(window)) {
			...
			VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &stagingBuffers.vertices, textureVertices.size() * sizeof(TextureVertex), textureVertices.data()));
			VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &stagingBuffers.indices, textureIndices.size() * sizeof(uint32_t), textureIndices.data()));

			// Device local destination buffers
			VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &vertexBuffer, textureVertices.size() * sizeof(TextureVertex)));
			VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &indexBuffer, textureIndices.size() * sizeof(uint32_t)));
