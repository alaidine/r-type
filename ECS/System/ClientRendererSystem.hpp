...
public:
  void SetPlayerTexture(Texture2D *texture) { m_playerTexture = texture; }

  void RenderClient(Core &core, Entity entity) {
    ...
    rec = {pos.position.x, pos.position.y, frameWidth * 2.0f,
           frameHeight * 2.0f};
    destrec = {pos.position.x, pos.position.y, frameWidth * 2.0f,
               frameHeight * 2.0f};

    DrawTexturePro(*m_playerTexture, sourcerec, destrec, origin, 0.0f,
                   sprite.color);
