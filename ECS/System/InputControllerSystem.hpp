...
void DrawMissiles(Position &pos, AnimationComponent &anim, Texture2D &player) {
  ...
  // }; Rectangle rec = { (float)pos.position.x, (float)pos.position.y,
  // frameWidth * 2.0f, frameHeight * 2.0f }; Rectangle destRec = {
  // (float)pos.position.x, (float)pos.position.y, frameWidth * 2.0f,
  // frameHeight * 2.0f }; Vector2 origin = { 0.0f, 0.0f };
...
void DrawPlayer(Position &pos, Texture2D &playerTexture) {
  ...
  Rectangle rec = {pos.position.x, pos.position.y, frameWidth * 2.0f,
                   frameHeight * 2.0f};
  Rectangle destRec = {pos.position.x, pos.position.y, frameWidth * 2.0f,
                       frameHeight * 2.0f};
  Vector2 origin = {0.0f, 0.0f};

  // NOTE: Using DrawTexturePro() we can easily rotate and scale the part of the
  // texture we draw source_rect defines the part of the texture we use for
  // drawing dest_rect defines the rectangle where our texture part will fit
  // (scaling it to fit) origin defines the point of the texture used as
  // reference for rotation and scaling rotation defines the texture rotation
  // (using origin as rotation point)
  DrawTexturePro(playerTexture, sourceRec, destRec, origin, 0.0f, WHITE);
  // DrawRectangleLinesEx(rec, 3, DARKBROWN);
}

void fire(Vector2 &missilePos, Vector2 &palyerPos,
          AnimationComponent &missileAnim) {
  missilePos.x = palyerPos.x;
  missilePos.y = palyerPos.y;
  missileAnim.rect.x =
      missileAnim._animationRectangle[missileAnim._current_frame].x;
  missileAnim.rect.y =
      missileAnim._animationRectangle[missileAnim._current_frame].y;
  missileAnim.rect.height =
      missileAnim._animationRectangle[missileAnim._current_frame].height;
  missileAnim.rect.width =
      missileAnim._animationRectangle[missileAnim._current_frame].width;
}
