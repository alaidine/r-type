...
void Client::InitECS(void) {
  ...
  registerBuilder
      .RegisterComponents<Position, Sprite, PlayerSprite, AnimationComponent,
                          InputController, Tag, MissileTag, playerCooldown,
                          NetworkedClient, LocalPlayerTag, RemotePlayerTag>(
          m_ecsCore);
  ...
  clientRendererSignature.set(m_ecsCore.GetComponentType<AnimationComponent>(),
                              true);
  clientRendererSignature.set(m_ecsCore.GetComponentType<NetworkedClient>(),
                              true);
...
void Client::UpdateClient(ClientState state) {
  ...
  if (it == m_clientEntities.end()) {
    TraceLog(LOG_WARNING, "UpdateClient: client_id %d not found",
             state.client_id);
    return;
  }
...
int Client::SendPositionUpdate(void) {
  ...
  msg.missile_count = std::min((unsigned int)m_missiles.size(),
                               (unsigned int)MAX_MISSILES_CLIENT);
...
void Client::DrawHUD(void) {
  ...
  DrawText(TextFormat("Connected: %s", m_connected ? "Yes" : "No"), 450, 450,
           32, MAROON);
}
...
void Client::UpdateAndDraw(void) {
  ...
  case IP_ADDRESS: {
    ...
    DrawRectangleLines((int)m_textBox.x, (int)m_textBox.y, (int)m_textBox.width,
                       (int)m_textBox.height, RED);

    DrawText(m_serverIp, (int)m_textBox.x + 5, (int)m_textBox.y + 8, 40,
             MAROON);
    DrawText(TextFormat("INPUT CHARS: %i/%i", m_letterCount, MAX_INPUT_CHARS),
             315, 250, 20, DARKGRAY);

    if (m_letterCount < MAX_INPUT_CHARS) {
      if (((m_framesCounter / 20) % 2) == 0)
        DrawText("_", (int)m_textBox.x + 8 + MeasureText(m_serverIp, 40),
                 (int)m_textBox.y + 12, 40, MAROON);
    } else
...
void Client::InitClient(char *serverIp) {
  ...
  if (!m_netClient->Connect(serverIp, PORT)) {
    TraceLog(LOG_WARNING, "Failed to connect to server at %s:%d", serverIp,
             PORT);
    m_disconnected = true;
    m_serverCloseCode = -1;
    return;
...
void Client::Fire(void) {
  ...
  missile.rect.height =
      m_missileAnimationRectangles[missile.currentFrame].height;
      ...
      std::remove_if(m_missiles.begin(), m_missiles.end(),
                     [](Missile missile) {
                       if (missile.pos.x > GAME_WIDTH) {
                         TraceLog(LOG_INFO,
                                  "Missile out of map... Deleting missile...");
                         return true;
                       }
                       return false;
                     }),
  ...
  for (Missile &missile : m_missiles) {
    ...
    if (missile.framesCounter >= (TARGET_FPS / missile.framesSpeed)) {
      ...
      missile.rect.height =
          m_missileAnimationRectangles[missile.currentFrame].height;
      missile.rect.width =
          m_missileAnimationRectangles[missile.currentFrame].width;
    }
    ...
    Rectangle sourceRec = {missile.rect.x, missile.rect.y, frameWidth,
                           frameHeight};
    Rectangle destRec = {(float)missile.pos.x, (float)missile.pos.y,
