...
Parallax::Parallax() : _gameWidth(GAME_WIDTH), _gameHeight(GAME_HEIGHT) {}

Parallax::~Parallax() { Cleanup(); }
...
void Parallax::Draw(void) {
  ...
  DrawRectangle(0, 0, static_cast<int>(this->_gameWidth),
                static_cast<int>(this->_gameHeight), BLACK);
  ...
  for (const ParallaxLayer &layer : this->_layers) {
    ...
    if (yOffset > 0)
      yOffset = 0; // Ne pas décaler vers le bas
    ...
    if (x2 + scaledWidth <= this->_gameWidth) {
      Rectangle destRec3 = {x2 + scaledWidth, yOffset, scaledWidth,
                            scaledHeight};
