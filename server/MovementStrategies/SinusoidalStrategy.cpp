...
SinusoidalStrategy::SinusoidalStrategy(float xSpeed, float amplitude, float frequency, float centerLineY, float startX)
    : _xSpeed(xSpeed), _amplitude(amplitude), _frequency(frequency), _centerLineY(centerLineY), _startX(startX)
{
}

Position SinusoidalStrategy::update([[maybe_unused]] const Position &currentPosition, float totalTime)
{
