...
DiveStrategy::DiveStrategy(float xSpeed, float diveDepth, float diveDuration,
                           float startX, float startY)
    : _xSpeed(xSpeed), _diveDepth(diveDepth), _diveDuration(diveDuration),
      _startX(startX), _startY(startY) {}

Position DiveStrategy::update([[maybe_unused]] const Position &currentPosition,
                              float totalTime) {
