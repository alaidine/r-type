...
CircularStrategy::CircularStrategy(float xSpeed, float radius, float angularSpeed, float startX, float centerY)
    : _xSpeed(xSpeed), _radius(radius), _angularSpeed(angularSpeed), _startX(startX), _centerY(centerY)
{
}

Position CircularStrategy::update([[maybe_unused]] const Position &currentPosition, float totalTime)
{
